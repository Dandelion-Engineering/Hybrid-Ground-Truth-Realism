...
├── Session Summaries/
│  └── HumanReport1.md … HumanReport37.md
└── Progress Reports/
...
- **`tools/probe_missing_depth_actual_null.py`** — independent pre-card evidence that the approved completed-data permutation null has an assumption-free, non-vacuous missing-depth interval: the completed vector length and missing source positions fix every seed-derived permutation before values are chosen. At 200 replicates the probe passes 8/8, returns `[12.254, 18.618] µm`, contains three actual completions, and reproduces the zero-missing approved null element-for-element. It uses synthetic arrays only and reads no archive, candidate asset or network data.
...
- `chats/Claude-Codex/Non-Finite Spike Depths/` is active. Rank 1 and a rank-2 holdout both carry sparse NaN depths with finite times. Claude accepted Codex's rejection of count-only exclusion and built a 59-check pre-card sensitivity module. Codex accepted its exact bin interval, support invariance and point-estimate propagation, plus Claude's correction to the first fixture, but returned the proposed fixed-arrangement null: the actual completed-`N` permutation is fixed and directly boundable. Claude owns that design correction before reader wiring or RC-005.
...
- No host is pinned and no candidate drift value exists. Rank 1's plan-only read completed and its real drift command stopped with no output because sparse NaN depths violate the still-binding input confirmation; a rank-2 diagnostic shows the same class. Support floors alone cannot authorize dropping them. Claude's stronger spread fixture proves the whole two-number gate can pass while missing depths decide the excursion, and Codex's completed-data-null probe proves the null can be bounded without a replacement counterfactual. Claude owns that pre-card repair; no RC-005 candidate, Rung 0, generation, or sorter run is authorized. The rank-5/7/9/13 declared-clock disagreements remain separately paused.
