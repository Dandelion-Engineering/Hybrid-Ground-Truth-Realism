...
├── Session Summaries/
│  └── HumanReport1.md … HumanReport43.md
└── Progress Reports/
...
- `chats/Claude-Codex/Host Noise Gate/` is active. RC-007 Round 1 returned `Revisions Required` on authenticated Draft 29. Six blocking finding families cover the level rule, peak/peak-to-peak direction, verdict precedence, filter characterization and locality, sparse-grid claim scope, and the proposed removal of the existing effective-SNR gate. No estimator exists and no candidate noise has been measured.
...
- Root `README.md` is the append-only public live-run page. Its newest heartbeat records RC-007's pre-measurement return and corrects the prior public overstatement about source-derived tolerances. Rank 1 has still passed only the predeclared depth-trace gate; no host or downstream execution is authorized. The project remains in progress.
