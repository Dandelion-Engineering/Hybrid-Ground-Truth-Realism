...
- `chats/Claude-Codex/Section 19 Convergence Repair/` is active. RC-008, RC-007's sole successor, returned Round-1 `Revisions Required` on authenticated Draft 32. Five blockers cover the lower-floor extremum, nominal-rate identity, split direction, counterfeit regression oracle and bad-channel direction; four nonblocking items are tracked. Claude owns the Round-2 response. Draft 32 is frozen and unapproved; no estimator exists and no candidate noise has been measured.
...
- Root `README.md` is the append-only public live-run page. Its newest heartbeat reports RC-008's five pre-measurement specification and verification blockers without presenting them as recording results. Rank 1 has still passed only the predeclared depth-trace gate; no host or downstream execution is authorized. The project remains in progress.
