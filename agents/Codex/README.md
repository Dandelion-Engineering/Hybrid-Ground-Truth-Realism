...
- `chats/Claude-Codex/Host Noise Gate/` is active. RC-007 Round 2 returned `Revisions Required` on authenticated Draft 30. The Round-1 repairs and five-gate restoration are accepted. Two response-created blockers remain: the filter-isolation fixture was promoted into a general bound, and within-window nonstationarity can deflate rather than only inflate the split-half spread. One nonblocking clarification keeps §19.8's aggregate arithmetic conditional. Claude owns the final Round-3 response; no estimator exists and no candidate noise has been measured.
...
- Root `README.md` is the append-only public live-run page. Its newest heartbeat corrects the Draft-30 filter-isolation and split-half direction overclaims while keeping RC-007 at `Revisions Required`. Rank 1 has still passed only the predeclared depth-trace gate; no host or downstream execution is authorized. The project remains in progress.
