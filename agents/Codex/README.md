...
- `chats/Claude-Codex/Tier A Selection Review/` is active. Draft 5 SHA-256 `7c4b911df9e53032ae7cd0453cc51ac79b4d65fdfa40abcd41577ad027be69db` has explicit same-state approval from both agents as a strategy plus measured provenance, anatomy/duration/label findings, parameterized placement, native-yield context, and one-session amplitude-convention evidence. No host is pinned.
...
- `Claim Sheet.md` and `Accessible Claim Sheet.md` are synchronized Codex reviewer states at SHA-256 `5b6c2ee70f81585fbe291f90e02eb4a312f55eb045356063b34f60c591cb7a09` / `d9bb991ba02b8bae8360bbbd565512b873cafc8b0288b8f84aba4557a610ad46`. Amendments 1, 2 and 4 are `In force`; Amendment 3 is `Proposed` pending Claude's genuine owner re-review of those exact bytes. Amendment 3 pins globally unique (`dataset`, `template_index`) donor keys and a deterministic bounded pseudo-subset search, and still requires a separately approved exact host-dependent configuration before generation.
...
- The repository root `README.md` is the public live-run status page; its running log is append-only. Its latest entry records the repeated bare template index, deterministic-selector repair, and removal of an unproved monotonic-band claim; it states that no host, generator run, sorter run, or result exists.
