...
├── Session Summaries/
│  └── HumanReport1.md … HumanReport41.md
└── Progress Reports/
...
- `chats/Claude-Codex/Rank 1 Drift Result/` is active. RC-006 is `Revisions Required` at Round 1: a fresh Step-11 run reproduced the committed report and JSON byte-for-byte and all executable evidence is green, but §18.2 omits the 59,040,736-byte range-reader cache term, overstates the resource headroom scale, and treats two working-set samples as an isolated byte-exact allocation measurement. The authenticated help output is 164 lines, not 165. Claude owns one response; Round 2 is delta-only.
...
- Root `README.md` is the append-only public live-run page. Its newest heartbeat is a forward correction narrowing the rank-1 claim to a pass of the predeclared depth-trace statistic, naming the 174 objects as label-blind units or clusters, and restoring the declared limits of the permutation diagnostic. The project remains in progress.
...
- RC-002 closed unapproved at its Convergence Decision, its one allowed successor RC-003 closed `Approved`, RC-004 closed `Approved` at Round 2, and RC-005 closed `Approved with Follow-Ups` at Round 2. RC-006 is open at `Revisions Required` Round 1: the rank-1 report reproduces exactly and its drift statistic passes, while three resource-report claims require repair. The strict all-finite-depth confirmation is superseded only as §17 declares: NaN is bounded as missing, either infinity remains fatal, and a decision-unstable completion bound pauses the candidate.
...
- No host is pinned. Rank 1's reproduced depth-trace result is `Delta_10min = 1.821 µm` with `Q95_null = 0.526 µm`; completion and reconciliation pass, but RC-006 remains open for reporting repairs and four other host gates remain open. Rank 2 is unmeasured. No Rung 0, generation or sorter run is authorized, and the rank-5/7/9/13 declared-clock disagreements remain separately paused.
