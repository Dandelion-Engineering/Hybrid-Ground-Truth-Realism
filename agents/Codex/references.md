...
**Buccino, A. P., Sridhar, A., Feng, D., Svoboda, K., & Siegle, J. H. (2026). Efficient and reproducible pipelines for spike sorting large-scale electrophysiology data. _eLife_, 15, RP110170.** [https://doi.org/10.7554/eLife.110170.3](https://doi.org/10.7554/eLife.110170.3) · [reviewed-preprint methods](https://elifesciences.org/reviewed-preprints/110170v2) · [pinned SpikeInterface 0.104.8 filter source](https://github.com/SpikeInterface/spikeinterface/blob/76c41846f88de3cc9dc5858d5c7f97dd6cb1955f/src/spikeinterface/preprocessing/filter.py) · [pinned margin reader](https://github.com/SpikeInterface/spikeinterface/blob/76c41846f88de3cc9dc5858d5c7f97dd6cb1955f/src/spikeinterface/core/recording_tools.py)
...
*How it informed the project:* Defines the control condition, the exact open question, the low-SNR stratum, the scale reference, and the need to use pairing rather than imitate a distributed benchmark on one workstation. For RC-007, the official methods text establishes a fifth-order 300-Hz Butterworth high-pass applied forward and backward. The pinned SpikeInterface 0.104.8 source implements `forward-backward` with `sosfiltfilt`, derives the automatic high-pass margin as five periods (16.667 ms and 500 samples at 300 Hz / 30 kHz), obtains that margin from real neighbouring samples, and removes it after filtering. It also designs the filter coefficients from `recording.get_sampling_frequency()` and converts the millisecond margin through that same rate. This controlled the Round-1 source correction, authenticated Draft 31's real-neighbour repair at its structural boundary, and shows that Draft 32's deliberate nominal-30-kHz operator is not exactly `FilterRecording.get_traces` on a recording whose declared rate differs from nominal; the rank-1 difference is tiny but nonzero. None of this turns the residual anchor-pipeline chunk-size dependence into a bound.
