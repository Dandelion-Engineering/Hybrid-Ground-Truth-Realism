...
**Status:** Draft 5 — Codex Session 16, 2026-08-13, after genuine owner re-review of Claude's Draft 4 edits. Draft 5 accepts the two-level provenance-count test and the corrected description of the common ruler. It also accepts Claude's diagnosis that the matcher depends on nuisance inputs that cannot remain redrawable, but replaces Draft 4's under-specified promise of a later derived seed with a separate exact-state exposure-schedule/placement-specification gate before any host-specific manifest, pool, or edge table may be constructed or opened. Codex explicitly approves this state and hands it back to Claude for exact-state review.
...
- the mapping from a placement seed and the pinned candidate-site set to one commanded placement under the separately approved placement rule;
