...
**Status:** Draft 3 — Codex Session 15, 2026-08-13, after genuine owner re-review of Claude's Draft 2 edits and after Amendment 6 entered force. Draft 3 enforces the source-count floor at every provenance stage, parameterizes the rule by the surviving target count `N = 10..16`, consumes one fixed target-eligibility manifest, keeps the removal set at the full injection-zone donor universe, and removes the erroneous exposure-weight percentage. Codex explicitly approves this state and hands it to Claude for exact-state review.
...
1. **Insertion stage plus source-count equality:** retain only edges whose candidate and target insertion identifiers are equal, and require a complete `N`-pair assignment whose selected controls use exactly `S_T` distinct `dataset` values. Equality is automatic for any complete insertion-blocked assignment, because every surviving target source must contribute its own controls.
2. **Session stage plus source-count equality:** reached only if stage 1 has no complete assignment satisfying the floor. Retain only edges whose session identifiers are equal, and require the selected controls to use exactly `S_T` distinct `dataset` values.
3. **Subject stage plus source-count equality:** reached only if stage 2 has no complete assignment satisfying the floor. Retain only edges whose subject identifiers are equal, and require the selected controls to use exactly `S_T` distinct `dataset` values.
4. **Unrestricted-edge stage plus source-count equality:** reached only if stage 3 has no complete assignment satisfying the floor. Permit every hard-eligible edge, and require the selected controls to use exactly `S_T` distinct `dataset` values.

If stage 4 admits no complete assignment, the pool state is infeasible. There is no fifth stage without the source-count equality.

**The floor is an equality, binds in both directions, and survives every provenance relaxation.** Slot 7 asks the arms to be balanced on the number of contributing source datasets, and its stated worry — "an arm drawn from five source datasets compared against an arm drawn from one is partly a provenance comparison wearing a region label" — is symmetric. A control set spread across more or fewer sources than T is rejected as an imbalance rather than accepted as extra diversity or unavoidable loss.

**That constraint is not an ordinary assignment constraint, and the implementation may not pretend it is.** Requiring an exact count of distinct sources over the selected set is a global cardinality condition that unconstrained min-cost flow and rectangular-assignment solvers do not enforce; a solver run without it can return an assignment the floor forbids, and a greedy repair returns one this rule does not define. At stages 2 through 4, the implementation must therefore enumerate candidate-source subsets of size `S_T` while requiring every source in an enumerated subset to appear at least once in the selected controls, or use another method that provably returns the Section 6 optimum under exactly that condition. For the current library there are at most 37 candidate sources and `S_T <= 4` for any surviving subset of the CA1 universe, so the largest source-subset search remains `C(37, 4) = 66,045`. Stage 1 may use the automatic equality above but must still assert it in the returned assignment.
...
- at every failed finer stage, the ordinary pairwise graph's maximum matching cardinality and the exact source-count-constrained feasibility result;
...
- the exact constrained source-set search method and optimality record at the selected stage;
...
- no complete `N`-pair assignment satisfying source-count equality at the unrestricted-edge stage;
