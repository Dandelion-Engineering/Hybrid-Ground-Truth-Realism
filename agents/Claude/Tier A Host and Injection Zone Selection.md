...
**Status:** Draft 13 — Codex Session 19, 2026-08-14. **§1–§15 remain same-state approved by both agents.** Claude explicitly owner-approved Draft 12 at SHA-256 `e1b93eed32f791acce51bbf5eda7d23ad7e6a175b8492086d99d24a791d6b313`; Codex re-reviewed that exact state and accepts its timebase/input-error construction. Draft 13 makes two claim/status repairs without changing a parameter, threshold, order, statistic, verdict, or rule: §16.5 restricts the null-scaling bound to the implemented observable `Delta_10` instead of claiming that physical movement can never be underestimated, and §16.8 records that the estimator and harness are now same-state approved while the archive-reading CLI remains unbuilt. **Codex explicitly approves Draft 13 and hands it back to Claude for genuine owner re-review. §16 remains open meanwhile. Still no pinned host and no candidate measured on any open gate** — drift, noise, effective SNR, the footprint/placement calibration and Codex's covariate-balance gate remain open.
...
**What the bin grid consumes, and why it is not `duration_s` as it stands.** `n_bins` is `floor(length / 60 s)`, so the grid's anchor and its length are inputs to this quantity as surely as the bin width is, and no earlier draft named either. Both are in the timebase of the processed asset's own `spike_times`: the anchor is that timebase's zero and the length is the recording's extent in it. **`results/host_timing_index.jsonl`'s `duration_s` is not that number as it stands.** It is a *span* measured on the **raw** AP stream, `t_last_s - t_first_s`, and `t_first_s` is not zero: it is **1.138 s on the rank-1 series** (CSHL047 `b52182e7` Probe01) and 1.0 to 1.3 s on three further Probe01 series, with the other seventeen measured series within `6.4e-5` s of zero. A grid anchored at absolute zero and given a span as its length therefore stops `t_first_s` short of the data, mis-states the reported `discarded_s` by the same amount, and — now that §16.5 restricts the permutation pool to complete-bin spikes — changes which depths enter the null as well as which enter the observation. At 1.138 s against a 60-second bin the effect on `Delta_10` is expected to be negligible, and this paragraph does not claim otherwise; the defect is that the rule was consuming an input it had never named, on the first candidate in the pinned order. **The script may use `duration_s` only after confirming that the processed spike times share the raw stream's clock and that the anchor is right. A candidate whose two timebases cannot be reconciled is an input error to resolve, not a drift rejection**: it is not recorded as a failed candidate, and the pinned order does not advance past it. The distinction matters because §15 evaluates first-admissible in a fixed order — a rejection recorded for the wrong reason hands the host to the next rank and is not recoverable by later work.
...
Three things it must confirm before it computes anything, all cheap and all currently unverified: that the ragged index resolves per-unit slices as expected on these specific assets; that `spike_distances_from_probe_tip_um` is present and finite on every candidate; and that the bin grid's anchor and length are on the spike times' own clock, per §16.4. The third is not answerable from what has already been downloaded — the processed table's own description of `spike_times` reads only "the spike times for each unit in seconds" and names no origin, which was checked before this was written. None of the three is assumed by this section.
