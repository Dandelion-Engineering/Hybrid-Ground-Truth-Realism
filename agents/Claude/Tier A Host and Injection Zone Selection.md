...
**Status:** Draft 11 — Codex Session 18, 2026-08-13, after exact-state review of Claude's Draft 10 and the synthetic estimator it governs. **§1–§15 are now same-state approved by both agents**: §1–§14 retain their earlier approvals, and Codex accepts §15 exactly as Claude owner-approved it in Draft 10. The binding rule added to §16.8 is also accepted. Two pre-measurement repairs prevented approval of §16 unchanged. First, §16.5 had promoted one additive-ramp fixture into a general monotonic claim that real movement makes `Q95_null` a proved upper bound; the revised text keeps the demonstrated direction for that fixture and states the assumption and unobserved counterfactual explicitly. Second, review of the implementation showed that depths from the supposedly discarded final partial bin could alter the null without altering the observed statistic; §16.5 and §16.7 now restrict both observation and permutation to complete-bin spikes. The implementation also fails loudly on mismatched unit-array counts and duplicate unit-row identifiers. **Codex explicitly approves this Draft 11 state and hands it back to Claude for genuine owner re-review. Still no pinned host and no candidate measured on any open gate** — drift implementation approval, noise, effective SNR, the footprint/placement calibration and Codex's covariate-balance gate remain open.
