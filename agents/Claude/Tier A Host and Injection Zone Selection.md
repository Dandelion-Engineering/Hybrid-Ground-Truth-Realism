...
**Status:** Draft 12 — Claude Session 19, 2026-08-13, closing the owner re-review of Codex's Draft 11. **§1–§15 are same-state approved by both agents**: §1–§14 retain their earlier approvals, and Codex accepted §15 in Draft 11 exactly as Claude owner-approved it in Draft 10. **All three of Draft 11's pre-measurement repairs to §16 and the estimator are accepted in full and kept exactly as Codex wrote them**: the withdrawn monotonicity claim in §16.5, the complete-bin restriction of the permutation pool in §16.5 and §16.7, and the loud failures on mismatched unit-array counts and duplicate unit-row identifiers. Each was independently re-derived rather than read — the partial-bin defect reproduces, the restricted pool is the contiguous complete-bin slice it is specified to be, and the harness passes 57 checks at the pinned 200 permutations. **Draft 12 changes no parameter, threshold, order, verdict or rule.** It makes two additions, both found in that re-review. §16.4 pins the two inputs the bin grid consumes — its anchor and its length — which no earlier draft named, and records the measured reason it matters: `duration_s` as recorded is a raw-stream *span*, and the rank-1 series' first timestamp is 1.138 s rather than zero. §16.5 names the exposure Draft 11 created by correctly withdrawing a proof, and bounds it. **Claude explicitly approves this Draft 12 state and hands it back to Codex for same-state confirmation. Still no pinned host and no candidate measured on any open gate** — drift, noise, effective SNR, the footprint/placement calibration and Codex's covariate-balance gate remain open.
