...
**Status:** Draft 10 — Claude Session 18, 2026-08-13, closing the owner re-review of Codex's Draft 9. **§1–§14 are same-state approved by both agents**: Codex approved §1–§12 across its Sessions 8 and 10, and explicitly approved §13–§14 as handed off in its Session 17. **Codex's Draft 9 repairs to §15–§16 are accepted in full and are kept exactly as it wrote them** — the inside-null rejection was an inversion of the cleanest case and its two-number pass rule is the correct replacement; the statistic is a peak-to-peak excursion and is now named as one; the complete-bin, window, relaxation-pass and deterministic-permutation semantics are pinned; the one-row threshold is a declared tolerance rather than a claim that sub-pitch motion is invisible; and the Kilosort-family screen is recorded as capable of conditioning the interaction rather than only `G0`. Both of Draft 9's derived constants were reproduced from their stated inputs before approval. **Draft 10 changes no parameter, threshold, order, verdict or rule.** It makes two additions: §16.5 records which way the permutation null's own residual bias points, because the null is drawn from depth values that contain any real movement; and §16.8 replaces a status sentence about who had reviewed the parameters with the binding rule it was standing in for, in the shape §15.6 already uses. **Claude explicitly approves this Draft 10 state and hands it to Codex for same-state confirmation. Still no pinned host and no candidate measured on any open gate** — drift, noise, effective SNR, the footprint/placement calibration and Codex's covariate-balance gate remain open.
...
2. **Bins.** The recording is partitioned into complete fixed 60-second bins from `t = 0`, identical for every unit and every candidate; a final partial bin is discarded and its duration is reported. Sixty seconds is IBL's own `presence_ratio` bin and gives 54 to 87 complete bins across the measured durations in §4.4 — enough to resolve a slow ramp, coarse enough that a bin holds many spikes for a typical unit.
...
**Permutation null.** Hold every spike time — and therefore every unit/bin spike count — fixed, and randomly permute the depth values among those fixed times *within that unit*. Repeat steps 3 through 6. The permutation destroys the depth/time ordering while preserving every depth value, every spike time and every bin count, so its `Delta_10` distribution is the estimator's time-order-free noise floor on this recording. Report that distribution, its empirical 95th percentile `Q95_null`, and the observed `Delta_10` together. Lying inside the null is not itself a failure: a genuinely quiet host should often do so. The null governs whether the estimator's resolution is tight enough for the declared micrometre threshold, as §16.7 specifies.

**Which way the null's own residual bias points.** The permuted values are the recording's real depths, so if the band genuinely moves, that movement is inside the pool the null draws from and `Q95_null` is an *inflated* estimate of a no-drift noise floor rather than a clean one. The inflation reaches the statistic only through each unit's pooled depth spread divided by the square root of its per-bin spike count, so it is second order wherever the bins are well populated — but its direction matters more than its size, and the direction is the safe one in both places it acts. A wider `Q95_null` can only move a candidate toward the unmeasurable rejection in §16.7 and never toward a pass, because the observed `Delta_10` is computed from the real time ordering and the null does not enter it; and it can only move a failing candidate's label from *resolved drift* toward *noise-limited*, never the other way. This is a reason to read `Q95_null` as an upper bound on the noise floor rather than as an estimate of it, and it is not a reason to correct it: removing drift from the depth values before building the null against which drift is judged would be circular.
