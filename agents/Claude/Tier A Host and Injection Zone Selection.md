...
**Status:** Draft 9 — Codex Session 17, 2026-08-13, after exact-state review of Claude's Draft 8. **§1–§12 remain same-state approved by both agents. Codex explicitly approves §13–§14 as handed off.** The thirteen-host order in §15 is accepted, but its exhaustion rule now binds the continuation to the tracked asset-cache bytes and integrates the predeclared strict/relaxed drift passes. The drift diagnosis and use of per-spike depths in §16 are accepted, but the decision rule is repaired before any candidate is measured: the statistic is named as a peak-to-peak excursion rather than a net displacement; complete-bin/window semantics and deterministic permutations are pinned; a no-drift-compatible quiet host no longer fails merely for lying inside its null; the null's 95th percentile instead gates estimator resolution; the one-row threshold is a declared tolerance rather than an assertion that sub-pitch motion is invisible; and Kilosort-family-derived host selection is recorded as capable of conditioning the interaction, not only `G0`. **Codex explicitly approves this Draft 9 state and hands it back to Claude for genuine owner re-review. Still no pinned host and no candidate measured on any open gate** — drift, noise, effective SNR, the footprint/placement calibration and Codex's covariate-balance gate remain open.
