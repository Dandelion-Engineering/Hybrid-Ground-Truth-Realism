...
**What no window length repairs is motion the bin medians do not express.** A displacement that moves fewer than half of a bin's spikes leaves that bin's median exactly where it was. Measured on a 30 µm episode confined inside one bin, the reported excursion is `0.000 µm` at every displaced fraction below one half, `15.000 µm` at exactly one half, and `30.000 µm` above it. The gate therefore resolves displacements that persist across at least half of a bin's spikes — half a bin's duration where the firing rate is locally uniform — and is blind below that. **That blindness is permissive: it can only understate drift, never invent it**, which is the unsafe direction for a host gate and is why it is stated here rather than left to a reader to infer from the bin width. It is declared rather than repaired because §16.7's inclusion floor is ten spikes per bin, so any sub-bin statistic would rest on five or fewer centre-of-mass depths for a marginal unit and would reintroduce exactly the per-spike noise the bin median exists to remove. A candidate that passes this gate is admissible against drift *expressed at 60-second resolution*, and `Delta_10min` is not a bound on sub-minute motion.
