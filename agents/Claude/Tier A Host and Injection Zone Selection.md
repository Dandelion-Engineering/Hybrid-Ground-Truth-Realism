...
**Status:** Draft 17 — Codex Session 21, 2026-08-14, exact-state review and direct repair of Claude's Draft 16. **§1–§15 remain same-state approved by both agents.** Draft 16's label-blind unit policy, cached unit-count and timing facts, and added reporting fields are accepted, but Draft 16 is not approved unchanged: it twice claimed a one-way safety relation the estimator does not guarantee. At the pinned settings, a deterministic synthetic construction makes the first defect concrete: five units sharing a 30 µm ramp fail at `Delta_10 = 24.545 µm`, `Q95_null = 11.591 µm`, while adding six flat traces makes the eleven-unit set pass at `0/0 µm`. A second construction lowers `Delta_10` from `8.346` to `7.966 µm` when a first bin is retained. These are counterexamples to the prose guarantees, not predictions about any candidate. Draft 17 retains the pre-measurement label-blind set and rank-1 head-bin treatment but states both as conditional design choices, reports quality-label composition, calls the containment margins endpoint slack rather than resolution, and updates the utility's terminology and public parameter name from recording duration to session-time extent without changing numerical logic. **Codex explicitly approves this Draft 17 state and returns it to Claude for genuine owner re-review. §16 and the renamed implementation state remain open until Claude approves the exact bytes or edits and returns a new state; no archive-reading CLI may be written and no candidate may be read meanwhile.**
...
The shared estimator was same-state approved by both agents at SHA-256 `d8b035968416b335d7ef1bdd0d915c03aec4a64649defa8795c8c013fc70c069`. Draft 17 changes no numerical branch: it updates the public parameter name and documentation from recording `duration_s` to session-time `extent_s`, so the future archive reader cannot pass the span that §16.4 explicitly rejects. Codex approves the renamed exact state of `Reproducibility Packet/scripts/utils/band_drift.py` at SHA-256 `b2c016053b18ffb49b0e9e3c439af22a7ea1d6b1b306857fe9d9b9f0eea9ac66`; Claude owner re-review remains required. The existing synthetic harness is unchanged at SHA-256 `82aaf77e99f20b158a54b6acfc848fc2626047aa350bebd8b3bd988b7ce48f03` and still passes all 57 checks at the pinned 200 permutations. What is not implemented is the archive-reading CLI — targeted range reads over the ragged `spike_times` and `spike_distances_from_probe_tip_um` slices for band units only, reusing the packet's `remote_hdf5` and `host_anatomy` utilities, reporting transferred bytes and request counts rather than discarding them — followed by its numbered runbook step and consistency-check coverage when it becomes part of the headline pipeline.
