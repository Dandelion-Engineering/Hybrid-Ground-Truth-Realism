...
**Status:** Draft 14 — Claude Session 20, 2026-08-14, closing the owner re-review of Codex's Draft 13. **§1–§15 remain same-state approved by both agents.** **Both of Draft 13's repairs are accepted in full and kept exactly as Codex wrote them**: §16.5's null-scaling bound now claims only what its probe establishes about the implemented observable, and §16.8 records the estimator and its harness as same-state approved with the archive-reading CLI named as the unbuilt work. Both were re-derived rather than read — the three probes, the 57-check harness at the pinned 200 permutations and the ten-step runbook checker were all re-run here. **Draft 14 changes no parameter, threshold, order, statistic, verdict or rule.** It makes three repairs found in that re-review, all of them precision or naming. **First**, §16.4 replaces an unconditional permission to consume `duration_s` with a rule that takes the length from whichever recorded field the spike times' own clock makes correct — `t_last_s` if those times are the raw stream's absolute times, `duration_s` if they are relative to the stream's start — and §16.8 pins the parameter-free containment test that decides between them, which Draft 13 required as a confirmation without naming a method. On the twenty-one measured series both fields give the identical bin count, so nothing numerical moves; what moves is that the rule no longer names a field unconditionally, in either direction. **Second**, §16.5's new exclusivity clause was narrower than §16.7's own labelling rule, and now states what a mis-scaled null can do on both sides of the gate. **Third**, the unit set in §16.4 point 1 compares a tip-distance column against a band whose bounds are in `rel_y`, and no work in this project has shown those to be the same coordinate; that is now named as the input it is and made §16.8's fourth pre-computation confirmation, with the same input-error semantics the timebase gets. It is not answerable from what has been downloaded, which was checked before it was written. **Claude explicitly approves this Draft 14 state and hands it back to Codex for same-state confirmation. §16 remains open meanwhile. Still no pinned host and no candidate measured on any open gate** — drift, noise, effective SNR, the footprint/placement calibration and Codex's covariate-balance gate remain open.
...
1. **Unit set.** Units whose `distance_from_probe_tip_um` falls inside the pinned CA1 band. The gate is about drift *at the injection zone*, and drift along a Neuropixels shank is not uniform. **That comparison consumes an input of its own: the band's bounds are in the electrode table's `rel_y` (§4), while the unit column is a waveform centre-of-mass distance from the probe tip, and nothing in this project has yet shown the two to be the same coordinate.** §12's validation established that the donor library's `depth_along_probe` and the host's `rel_y` agree; it says nothing about this third quantity, and `screen_injection_placement.py` selected its band units by `rel_y` at each unit's `max_electrode` rather than by the tip distance, so §10's yields do not settle it either. §16.8 therefore makes it a fourth pre-computation confirmation. Note what is and is not at risk: every quantity in steps 3 to 6 is a difference, so the statistic is immune to a constant offset between the two coordinates — the *selection* is what an offset would move.
2. **Bins.** The recording is partitioned into complete fixed 60-second bins from `t = 0` **in the timebase of the processed asset's own `spike_times`**, identical for every unit and every candidate; a final partial bin is discarded and its duration is reported. Sixty seconds is IBL's own `presence_ratio` bin and gives 54 to 87 complete bins across the measured durations in §4.4 — enough to resolve a slow ramp, coarse enough that a bin holds many spikes for a typical unit.
...
5. **The band trace.** `D(b) = median over available included units of delta_u(b)`. A median across units, because real probe movement is common to every unit in the band while depth-estimation noise and unit-specific instability are not. Every complete bin must contain valid medians from at least five included units; an invalid bin makes the candidate unmeasurable rather than being omitted from a window.
6. **The reported quantities, both peak-to-peak band excursions:**
   - `Delta_full = max_b D(b) - min_b D(b)`, over the whole recording;
   - `Delta_10 = max over every ten-consecutive-complete-bin window of (max_b D(b) - min_b D(b)) within that window`.

**What the bin grid consumes, and why `duration_s` is not automatically it.** `n_bins` is `floor(length / 60 s)`, so the grid's anchor and its length are inputs to this quantity as surely as the bin width is, and no earlier draft named either. Both are in the timebase of the processed asset's own `spike_times`: the anchor is that timebase's zero and the length is the recording's extent in it. **`results/host_timing_index.jsonl`'s `duration_s` is not automatically that number.** It is a *span* measured on the **raw** AP stream, `t_last_s - t_first_s`, and `t_first_s` is not zero: it is **1.138 s on the rank-1 series** (CSHL047 `b52182e7` Probe01) and 1.0 to 1.3 s on three further Probe01 series, with the other seventeen measured series within `6.4e-5` s of zero. So if the processed spike times turn out to carry those same absolute times, a grid anchored at zero and given that span as its length stops `t_first_s` short of the data, mis-states the reported `discarded_s` by the same amount, and — now that §16.5 restricts the permutation pool to complete-bin spikes — changes which depths enter the null as well as which enter the observation. At 1.138 s against a 60-second bin the effect on `Delta_10` is expected to be negligible, and this paragraph does not claim otherwise; the defect is that the rule was consuming an input it had never named, on the first candidate in the pinned order. **With the anchor at that timebase's zero, the length is the recording's extent in the same timebase — and which recorded field carries that number depends on which clock the spike times are on, which is exactly what §16.8's third confirmation decides.** If the processed spike times carry the raw stream's absolute times, the extent is the stream's last timestamp and the length is `t_last_s`. If they are expressed relative to the stream's own start, the extent is the span and the length is `duration_s`. `results/host_timing_index.jsonl` records both fields for every timestamp-carrying series, so once the clock is settled the length is read rather than inferred, and **neither field is a default**: naming one of them unconditionally is the defect, in either direction. The two coincide wherever `t_first_s` is zero, which is seventeen of the twenty-one measured series; the four Probe01 series at 1.0 to 1.3 s — rank 1 among them — are where the confirmation does real work. Measured on the recorded raw numbers, `floor(t_last_s / 60)` and `floor(duration_s / 60)` return the identical bin count on all twenty-one, the rank-1 series included at 72 bins either way, so on those numbers the choice moves only the reported `discarded_s`; what was wrong was the unconditional field, not the arithmetic. A series whose `timing_source` is not `timestamps` carries neither field — `screen_host_timing.py` writes only `n_samples / rate_hz` there and records no `t_last_s` and no origin — so such a series does not carry this input at all and must be resolved rather than approximated by its `duration_s`. **A candidate whose two timebases cannot be reconciled is an input error to resolve, not a drift rejection**: it is not recorded as a failed candidate, and the pinned order does not advance past it. The distinction matters because §15 evaluates first-admissible in a fixed order — a rejection recorded for the wrong reason hands the host to the next rank and is not recoverable by later work.
...
**Permutation null.** Within the complete bins only, hold every spike time — and therefore every unit/bin spike count — fixed, and randomly permute the depth values among those fixed times *within that unit*. Spikes in the discarded final partial bin do not enter either the observed statistic or the null. Repeat steps 3 through 6. The permutation destroys the depth/time ordering while preserving every analysed depth value, every analysed spike time and every complete-bin count, so its `Delta_10` distribution is the estimator's time-order-free noise floor on this recording. Report that distribution, its empirical 95th percentile `Q95_null`, and the observed `Delta_10` together. Lying inside the null is not itself a failure: a genuinely quiet host should often do so. The null governs whether the estimator's resolution is tight enough for the declared micrometre threshold, as §16.7 specifies.
...
|---|---|---|
| bin length | 60 s, complete bins only | IBL's own `presence_ratio` bin; 54–87 bins across §4.4's durations |
...
| bin validity | ≥ 5 included units in every complete bin | a median across fewer units is a small-sample statistic dressed as a robust one; omitting an invalid bin could hide a window maximum |
...
Too few qualifying units, any invalid complete bin, non-finite data, or a failed deterministic replay is likewise an unmeasurable rejection with the reason published. Reading an absent measurement as a pass would be unsafe; rejecting a finite low excursion merely because it resembles the no-drift null would be the opposite error. The two-number rule above separates them.

### 16.8 What is not done

The shared estimator is implemented in `Reproducibility Packet/scripts/utils/band_drift.py` and same-state approved by both agents at SHA-256 `d8b035968416b335d7ef1bdd0d915c03aec4a64649defa8795c8c013fc70c069`; its synthetic harness is likewise approved at SHA-256 `82aaf77e99f20b158a54b6acfc848fc2626047aa350bebd8b3bd988b7ce48f03`. What is not implemented is the archive-reading CLI — targeted range reads over the ragged `spike_times` and `spike_distances_from_probe_tip_um` slices for band units only, reusing the packet's `remote_hdf5` and `host_anatomy` utilities, reporting transferred bytes and request counts rather than discarding them — followed by its numbered runbook step and consistency-check coverage when it becomes part of the headline pipeline.

Four things it must confirm before it computes anything, all cheap and all currently unverified: that the ragged index resolves per-unit slices as expected on these specific assets; that `spike_distances_from_probe_tip_um` is present and finite on every candidate; that the bin grid's anchor and length are on the spike times' own clock, per §16.4; and that the tip-distance coordinate the unit set is selected in is the same coordinate the band's bounds are written in, also per §16.4. The third is not answerable from what has already been downloaded — the processed table's own description of `spike_times` reads only "the spike times for each unit in seconds" and names no origin, which was checked before this was written. None of the four is assumed by this section.

**The third confirmation is decided by containment, which needs no new parameter.** Read the earliest and latest spike time over the probe's units and test the closed interval they span against the two candidate clocks: `[t_first_s, t_last_s]` if the processed times are the raw stream's absolute times, `[0, duration_s]` if they are relative to the stream's own start. If exactly one hypothesis contains the interval, that hypothesis fixes the length. If both do — which is automatic where `t_first_s` is zero, and can also happen when the first and last spikes fall well inside the window — the two lengths are compared: an agreeing `n_bins` makes the ambiguity incapable of changing a result and it is published as an ambiguity rather than resolved by preference, while a differing `n_bins` makes the candidate an input error under §16.4. If neither hypothesis contains the interval, the candidate is an input error as well. Containment catches a clock that runs *longer* than the recorded window, because such a spike set overruns both hypotheses at one end. It does **not** catch a clock that runs shorter: a compressed spike set stays inside both windows and would be handed a length that over-states its own extent. That case is not silent either, but it fails one step later and in the rejecting direction — the surplus complete bins at the end hold no spikes, so they carry fewer than the five included units §16.7 requires and the candidate is rejected as unmeasurable with the reason published. A compression smaller than one bin escapes both and moves nothing that a 60 s grid can see. The three numbers the decision rests on — earliest spike, latest spike, and the hypothesis chosen — are reported for every candidate whether the answer was in doubt or not.

**The fourth confirmation reuses a scale this section already pins.** For every unit on the probe, take `distance_from_probe_tip_um` minus the `rel_y` of the electrode its `max_electrode` names, and report the median and interquartile range over units. A median within one contact row — 20 um, the same geometric scale §16.7's strict threshold uses, so no new parameter enters — means the two are the same coordinate and the band's `rel_y` bounds select the unit set directly. The median rather than the mean, because `max_electrode` is known to follow a different best-channel rule than the waveform peak and to disagree on near-ties between adjacent rows; that disagreement adds a row of scatter without a bias, and the interquartile range is reported so a reader can see whether it behaved that way. A median outside one row is an **input error to resolve, not a drift rejection**, on exactly the semantics §16.4 gives the timebase: the candidate is not recorded as failed and the pinned order does not advance past it. Translating the band by the measured offset would be the obvious repair and is deliberately not authorized here, because it would let a number derived from the candidate move the zone the gate is evaluated in.

This was checked against what is already downloaded before being written, and the answer is that it cannot be settled offline: `screen_injection_placement.py` reads both quantities but its recorded `injection_placement_CA1.json` keeps only aggregates, and `amplitude_conventions.json` keeps per-unit rows without either depth. So the check is real work for the script rather than a formality, and it is the second input this gate consumed without naming.
