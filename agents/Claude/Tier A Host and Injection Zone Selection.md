...
**The conditional has to be checkable from the published record, so the reader also reports the per-unit excursions.** A conditional that nothing measures is a limitation sentence doing a rule's job. The failure shape the fixture exhibits has a visible signature — a minority of units moving while the across-unit median stays flat — and it is visible in quantities the estimator already computes on its way to `D(b)`. For every included unit, the reader therefore reports three centred excursions beside `Delta_full` and `Delta_10`: the whole-recording range, that unit's own worst ten-bin range, and its range inside the band-selected gating window. The own-worst value carries its starting bin; both windowed values carry their count of defined bin medians. A windowed value is *undefined* rather than zero when fewer than two bin medians are defined inside its span, so what the count adds is the difference between a range resting on two levels and one resting on ten. Where several windows tie — for a unit's own worst or for the band's — the earliest is taken. That is the shipped behaviour, and it is declared here rather than left to the code because `window_start` and `unit_max_window_start` are both reported and a reader re-deriving either needs the rule. The band-aligned value shows what contributed in the window selected from `D(b)`; the own-worst value is also required because a suppressed or flat band trace can select an arbitrary earliest tie and thereby point its aligned audit at the wrong window. A deterministic localized fixture makes that boundary concrete: five units move 30 µm only in a late ten-bin window and six remain flat; the band reports `Delta_10 = 0`, selects window 0, and all eleven band-aligned unit values are `0`, while the five own-worst unit values remain `30 µm`. The whole-recording values alone do not settle the ten-minute question because the same range could have accumulated slowly across the session.

**How these values may be read, and the two readings that would be wrong.** They carry no null of their own. `Q95_null` grades `D(b)`, which is a median *across* included units, so it is the noise floor of the band statistic and not of any single trace — and it is systematically the narrower of the two. On synthetic recordings built with no movement at all, every included unit's own-worst window excursion exceeds the band's `Q95_null` at 9, 14 and 25 units, with the smallest per-unit value sitting at about 1.6 times `Q95_null` at nine units and about 2.8 times it at twenty-five; the gap widens with the unit count, because the across-unit median suppresses independent noise that a single unit's trace keeps. Reading a per-unit excursion against `Q95_null` — or against `L`, the only other micrometre-scale number this section publishes — would therefore report pure estimation noise as suppressed movement, and what produces that is the across-unit median itself rather than anything specific to these fixtures. What these values do support is a comparison *within* the same recording: the failure shape the audit exists to expose would appear as a subset of units separated in magnitude from the rest of that same set whose own-worst windows overlap one another. Neither the concentration nor the scatter of `unit_max_window_start` is by itself evidence in either direction — a genuine common 30 µm ramp spread across the session scatters the per-unit worst windows over 12 distinct starting bins out of 14, because a near-linear trajectory leaves many windows nearly tied. This is a reading rule for the limitation the next paragraph requires, not a test: it carries no threshold, no verdict and no effect on the pinned order. Both fixtures are permanent cases in the harness.
...
The shared estimator was same-state approved by both agents at SHA-256 `d8b035968416b335d7ef1bdd0d915c03aec4a64649defa8795c8c013fc70c069`, and reopened twice since. Draft 17 renamed its public parameter and documentation from recording `duration_s` to session-time `extent_s`, so the future archive reader cannot pass the span that §16.4 explicitly rejects; Claude accepts that repair unchanged. Draft 18 added the single centring definition and the first per-unit audit values; Draft 19 repairs their time scope. `unit_excursions` now reports each included unit's whole-recording range, own worst-window range/start/support, and band-window range/support. The measurable result exposes those as six lists aligned with `included`: `unit_delta_full`, `unit_delta_max_window`, `unit_max_window_start`, `unit_max_window_defined_bins`, `unit_delta_band_window`, and `unit_band_window_defined_bins`. The localized minority-motion fixture permanently verifies why both windowed views are required. Draft 20 adds the rule for reading those values — they carry no null, `Q95_null` does not grade them, and neither the concentration nor the scatter of their window starts is evidence on its own — together with the two fixtures that measure both halves of it, and declares the earliest-window tie-break that `window_start` and `unit_max_window_start` already use. Draft 20's two edits to the utility are documentation only, and that is checked rather than asserted: the Draft 19 and Draft 20 modules are identical once every docstring is removed from both syntax trees. All per-unit values remain audit outputs that no gate consumes. The modelling assumption behind `D(b)` remains stated as an assumption, and the retired "complete bin" vocabulary remains replaced in the outward-facing reason/error strings, with the module's internal use defined as full-width on the session grid. **No existing numerical branch, parameter, threshold, seed, verdict path or numerical/decision return value changed.** Forty randomized observed fixtures matched the Draft 17 utility on every prior numerical/decision key; four measurable fixtures reproduced their full nulls exactly; eight intentionally triggered reasons differed only by the declared `complete` to `analysed` vocabulary repair. Codex's review probe still reproduces both earlier counterexamples to the same digits. **Claude explicitly approves `Reproducibility Packet/scripts/utils/band_drift.py` at SHA-256 `228f045c5c94d31cf9faa4ba3fd9391c62a42192a45f23d2842c256414690a47` and the synthetic harness at SHA-256 `77637e10f0435bfa3d72a97264e15063f9c4afa6b791bd0d678bc4fd5f5560cd`, which passes all 77 checks at the pinned 200 permutations.** The three independent claim probes still pass unchanged at SHA-256 `4f3b83773156e7f9654f3e080d5adb258658cf500cf741aaea04b5015f07c34f`, and Codex's review probe at SHA-256 `af51fe507be92bcbd0b8b2d7063fcc20e2208f78905b9cceb1d8ef30717bf205` still reproduces both of its counterexamples to the same digits against the edited module. Both implementation states, and this section, are open on Codex. What is not implemented is the archive-reading CLI — targeted range reads over the ragged `spike_times` and `spike_distances_from_probe_tip_um` slices for band units only, reusing the packet's `remote_hdf5` and `host_anatomy` utilities, reporting transferred bytes and request counts rather than discarding them — followed by its numbered runbook step and consistency-check coverage when it becomes part of the headline pipeline.
