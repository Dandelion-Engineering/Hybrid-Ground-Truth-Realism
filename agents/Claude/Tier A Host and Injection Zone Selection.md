...
3. **High-pass at 300 Hz, with the anchor pipeline's own operator, followed by a margin.** A fifth-order Butterworth high-pass, designed at the nominal 30,000 Hz in second-order-section form, applied forward and backward with `scipy.signal.sosfiltfilt` at `padtype="odd"` and `padlen=18`, which is zero phase. This is not merely similar to the anchor pipeline's filter; it is the same operator. SpikeInterface's `FilterRecording` defaults are `filter_order=5`, `ftype="butter"`, `filter_mode="sos"` and `direction="forward-backward"`, and that direction resolves to `sosfiltfilt` for an `sos` filter. `padlen=18` is scipy's own default for this particular `sos` and is pinned here **by value**, so that a later scipy whose default changes cannot move the estimate silently. Then discard **500 samples (16.667 ms)** at each end, retaining **12,020** samples. That margin is the anchor's own rule rather than a width chosen here: `highpass_filter`'s default `margin_ms="auto"` resolves to `5 × (1000 / freq_min)` milliseconds, which is 16.667 ms at a 300 Hz corner and exactly 500 samples at 30 kHz.
...
**The filter is designed at the nominal 30,000 Hz rather than at each candidate's own measured rate**, so that one operator is applied to every candidate rather than thirteen slightly different ones. The cost of that choice is bounded by a measured quantity rather than by an assumption: across the twenty-one AP series in `Reproducibility Packet/results/host_timing_index.jsonl` the measured rate differs from nominal by at most `9.946e-06` relative, which moves the 300 Hz corner by at most **0.003 Hz**.

**Three declared differences from the anchor chain, each with its direction stated rather than waved at.** Draft 29 declared a different third one — a brick-wall filter standing in for the pipeline's Butterworth — and it is gone rather than better hedged; §19.11 records why.
...
**The margin is taken from inside the window rather than from real neighbouring samples, and this is the one deviation carrying a measured bound rather than a direction.** The anchor pipeline filters a chunk together with real data on either side and strips the margin afterwards. This gate reads one HDF5 chunk and cannot obtain neighbours without paying for another chunk, so the filter's start and end transients are driven by scipy's odd padding rather than by the recording. The transient decays with the filter's slowest pole, and `agents/Claude/tools/probe_filter_chain.py` measures rather than assumes it: pole radius `0.980781307`, time constant **51.531 samples (1.718 ms)**, so a 500-sample margin is **9.703 time constants** and leaves a residual factor of `6.1e-05`. Measured end to end — the same window filtered in isolation against that same window filtered inside nine chunks of continuous signal, worst case over twelve synthetic recordings — the retained core's scale estimate differs by **`+1e-06` relative** and no retained sample by more than **0.0006 µV**. That is the entire deviation.
...
2. **Windows, and the grid they sit on.** `K` disjoint chunk-aligned windows, each exactly one HDF5 chunk of 13,020 samples. Let `C` be the number of chunks lying entirely inside the AP extent. The grid `G` is the chunk indices

   `i_k = floor( k · (C − 1) / (K − 1) + 0.5 )`, for `k = 0 … K−1`

   — written as an explicit `floor(x + 0.5)` rather than as a language's `round`, because Python's rounds halves to even and a grid may not depend on that. `i_0 = 0` and `i_{K−1} = C − 1`, so the grid spans the whole full-chunk extent instead of stopping short of its tail. Deterministic, seedless, and fixed by the file's own layout rather than by anything a session chooses after looking. On rank 1, `C = 9,999`, the indices run `0, 169, 339, …, 9,829, 9,998`, and the final 1,020 samples fall in a partial chunk that no window uses.

   **What the grid guarantees, and what it does not.** Consecutive indices differ by at most `g = 170` chunks on rank 1, so the longest run of consecutive *unsampled* chunks is `g − 1 = 169`. Any interval that fully contains `g + 1 = 171` consecutive whole chunks — **74.214 s** at rank 1's 0.434 s chunk — therefore fully contains at least one sampled window, anywhere inside the grid's span. That is the coverage this design licenses, and it is the only coverage it licenses: **an excursion shorter than that can fall entirely between two windows and is invisible to every quantity in this section.** Sixty windows are 26.04 s, **0.600%** of rank 1's extent. `g`, the guaranteed-detection duration and the realized coverage fraction are computed and published per candidate under §19.7 rather than inherited from this paragraph, because `C` varies.
3. **Per-channel, per-window scale.** `σ̂_c(k)` is §19.3's estimate on band channel `c` in window `k`, after the margin discard.
...
| windows | `K = 60`, at chunk indices `floor(k·(C−1)/(K−1) + 0.5)` | 26.04 s of coverage spanning rank 1's whole full-chunk extent; guarantees detection of any excursion fully containing 171 consecutive chunks (74.214 s); §19.4, at a projected 319 MB of transfer; §19.9 |
| margin | 500 samples (16.667 ms) at each end, after the filter | SpikeInterface's own `margin_ms="auto"` rule, `5 × (1000/freq_min)`, evaluated at a 300 Hz corner |
...
| split | two disjoint halves of the retained 12,020 samples | §19.5 |
| **level floor** | **1.25 µV, strict and relaxed alike** | derived below; it is not part of the ladder and does not relax |
...
So the admissible band is **`1.25 µV ≤ sigma_worst_sampled ≤ N`**, with `N = 10.0 µV` in the strict pass and the upper bound the operative one — **and both ends reach a verdict.** Draft 29 declared this interval and then wrote a pass rule that tested only its ceiling, which left the floor as a limitation sentence doing a rule's job; §19.6's branch list below is where the floor now decides something. **The floor does not relax.** It is derived from the saturation condition rather than from the SpikeForest ladder, and the relaxed pass moves `N` alone.
...
**What is kept.** The host-aggregate quantity is `A / sigma_worst_sampled` for `A` in the pinned target range, and the two conditions a *host-aggregate* gate could impose on it — a floor on the quietest injected unit and a ceiling on it — are the two inequalities §19.6 already rearranged into bounds on `sigma_worst_sampled`. Evaluating them a second time under a different name would be bookkeeping rather than evidence. This section therefore **reports** `snr_p2p_min = A_min / sigma_worst_sampled` and `snr_p2p_max = A_max / sigma_worst_sampled` beside the level verdict and records that they carry **no independent rejection power**. That is a statement about two numbers, not about a gate.
...
- **σ̂ is not the σ the sorting pipeline will compute.** Phase shift is omitted and bad channels are not masked. The first biases σ̂ upward; the second inflates `R_space_sampled` in both directions. The filter is no longer among the deviations — it is the anchor's own operator at the anchor's own margin — and what remains of that difference is one measured bound: a margin drawn from inside the window rather than from real neighbours, worth `+1e-06` relative on σ̂.
...
- **`sigma_worst_sampled`, `R_space_sampled` and `R_null_sampled` are maxima over sixty sampled windows, not over the recording.** The grid guarantees that any excursion fully containing 171 consecutive chunks — 74.214 s at rank 1 — is caught; anything shorter can fall entirely between windows and is invisible to all three. Nothing in this section licenses a claim about unsampled time.
- **The `snr_p2p = 40` ceiling is judgement, not literature.** SpikeForest's own finding is that the accuracy/SNR relation is sorter-dependent, which is a reason no published number pins saturation, and this section does not manufacture one. It is declared, it is not expected to bind, and Round 1 did not challenge it — which is a fact about that round rather than a defence of the number, and it stays labelled.
...
- **This section has been reviewed once and repaired once.** Draft 29 was handed to Codex under Review Card RC-007 as a specification, before any estimator was written, so that the sequence §16 followed — specify, review, then implement — is the sequence this gate follows too. Round 1 returned `Revisions Required` with six blocking finding families; §19.11 records what each one changed. **This Draft 30 state is not approved by anyone yet**, and RC-007 Round 2 is delta-only.
