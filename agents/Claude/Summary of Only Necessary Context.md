...
**Rewritten at the end of Claude Session 30 · 2026-08-15 09:41 PDT**
**Next session is Claude Session 31. No count-based progress report is due** (the next is Session 32). A phase transition or an approved amendment written in your session would trigger one anyway.
...
***Convergence in place of escalation* has now been used once, and it worked.** `Escalated` is not an outcome; the outcomes are **Approved · Approved with Follow-ups · Revisions Required · Split/Redesign Required**. At either trigger (a second LATE-BLOCKER or any new blocker after Round 2; or the third round-trip without both approvals) the disputed state freezes and the agents run **one agent-only Convergence Decision**: each writes, once, the minimum claim it thinks can ship, the evidence that controls, the strongest evidence against its own position, and one acceptable safe disposition. **Evidence determines what may ship; consensus determines what happens next. Underdetermined evidence is not resolved in favour of approval.** The card closes at a disposition, the owner repairs **outside** review, and may open **one** successor card with a `Supersedes:` line; **a second like-for-like successor is not allowed**.

**RC-001 closed `Approved`. RC-002 closed `Revisions Required` at Session 30 by the first Convergence Decision — both statements are on the card and both agents explicitly approved the disposition. RC-003 is its one permitted successor and is open on Codex, Round 1.** Clause 5 is live on RC-003: if it also fails to converge on the same scoped purpose, the work must be split or redesigned and **no third like-for-like card may open.**
...
**⚠️ This file describes the moment it was written.** Codex has now nine times posted a handoff within the hour after a session closed. **Read the active chats before you act on §2.**
...
| `Reproducibility Packet/scripts/utils/archive_units.py` | **`c1050b07…` — open on Codex, RC-003 Round 1. CHANGED in Session 30.** |
| `Reproducibility Packet/scripts/measure_host_drift.py` | **`0f9c4ad1…` — open, RC-003 Round 1. CHANGED in Session 30.** |
| `agents/Claude/tools/test_measure_host_drift.py` | **`4b430838…` — open, RC-003 Round 1. CHANGED. 279 checks, 0 failed, 29.4 s.** |
| `agents/Claude/tools/mutate_rc002_repairs.py` | **`f9e0d732…` — open, RC-003 Round 1. CHANGED. 16 of 16, control green.** |
| `agents/Claude/tools/mutation_test_runbook_checker.py` | **`ea85ede2af89fa18e1cf41633c53bc9a96ee0cd6f6190b0394b02afd4a4678fc` — unchanged since Session 29; open, RC-003 Round 1. 18 of 18, control green.** |
| `Reproducibility Packet/scripts/check_runbook_consistency.py` | **`848e6d033a424d8a280519765244ed32329dbd53f52594da8cc700310a776c9f` — unchanged; open, RC-003 Round 1.** |
| `Reproducibility Packet/README.md` | **`ae01b1a2b766a22a25ed0ddf2dc0235bc61e8254045e46655457da2d2cf2d4b5` — unchanged; open, RC-003 Round 1.** |
| `agents/Codex/tools/probe_rc002_round3.py` | `506d7280f7dbcc98ebc9e0ca544195c9dcfe819eca19e5e6f6b41cfa9adc5e15`. **His.** Against the frozen RC-002 state it returns exit 0 with his figures; **against the Session 30 state it returns exit 1 with both underbound flags False. That is the repair.** |
| `agents/Codex/tools/probe_rc002_round2.py` / `probe_rc002_round1.py` | `ea806c59…` / `e4197bca…`. His. Both still demonstrate their repairs (round2 raises; round1 four FAIL and the fifth raises). |
| `agents/Codex/tools/probe_rc001_round1.py` / `probe_draft16_safety_claims.py` | Both take `--repo-root`; `probe_rc001_round1.py` requires it. 0 failures / digits unchanged at Session 30. |

**The full digests of the four changed files are in `Review Cards/RC-003 ….md`. Use the card, not this table, when you need all sixty-four characters.**
...
- `chats/Claude-Codex/Bounded Archive Read Review/` — **RC-003 Round 1 is open on Codex and is a FULL seven-file pass, not a delta.**
- `chats/Claude-Codex-Human/Review Method Change/` — **active and stays active.** Three observations were posted at Session 30; the Session-29 queued note went in with them.
- **All five Tier A / packet / RC-002 chats are concluded**, including `Archive-Reading Drift Command Review/`, which now carries a `Summary.md`.
...
**If Codex returns findings, respond under the ordinary method** — RC-003 is a fresh card with its own three round-trips. **If it reaches a non-approval disposition, clause 5 binds: no second like-for-like successor.** The work is split or redesigned with the changed boundary named.

## 3. What Session 30 did

**RC-002's Convergence Decision, then the repair outside review, then RC-003.**

- **The blocker, RC-002-F1-R2.** `read_band_units` enforced `max_bytes` against `peak_resident_bytes` and *then* called `source_provenance(handle)`, which reads each complete stored provenance dataset with `node[()]`. A schema-valid fixture with a 4,200,030-character `general/source_script` was admitted at `peak_resident_bytes = 267,001` under a `174,368`-byte transfer bound and then transferred and retained `4,232,336` bytes. **I reproduced Codex's probe unmodified before writing anything and agreed with his disposition without counterproposal.**
- **The repair, four parts.** (1) The provenance read **moved into preflight**, before `remote.n_bytes` is captured, so its cost is in `spent_bytes` → `cache_bound_bytes` → `peak_resident_bytes`; its dict goes into `held=` so its strings are in `structures_bytes`. (2) **`PROVENANCE_MAX_BYTES = 65536`, pinned in the module.** Where the stored size is meaningful an oversized value is **not read**; for a variable-length string it is not meaningful — **h5py 3.16.0 reports 16 bytes of storage and 8 of `nbytes` for a 4.2 MB value**, because the characters are in the global heap — so there the value is read in preflight and **retained only up to the cap** with a self-describing marker. (3) **The ceiling's scope is stated in the command** — it covers the processed read and everything inside it, and not the two raw-asset reads, which are bounded by construction and reported separately. (4) **The general rule became a whole-suite invariant** (§4.1).
- **The invariant's first version was wrong, in the mirror direction.** It compared `record["io"]["processed_units"]["bytes"]` — which the local reader increments on every read, including re-reads — against a bound on **distinct** bytes. It reported an `84,144`-against-`81,360` violation on **eighteen** cases that was pure double counting; both figures sat at the fixture's own file size, which is what gave it away. **A test written to prevent a false pass produced a false failure, and that is not the safe direction either.**
...
`archive_units.py` public surface: `read_flat_electrodes` · `column_descriptions` · **`source_provenance(handle, max_bytes=PROVENANCE_MAX_BYTES)`** · `read_integer_column` · `read_unit_scalars` · `check_ragged_alignment` · `resolve_unit_electrodes` · `select_band_units` · `chunk_byte_ranges` · `column_layout` · `python_structure_bytes` · `band_slices` · `plan_transfer` · `read_band_units` · `electrode_tables_agree`. Private helpers added in Session 30: **`_stored_value_bytes`** and **`_capped`**.
- **`PROVENANCE_MAX_BYTES = 65536`** is a module constant and is deliberately not a parameter of the command.
...
- **`plan_transfer(band_units, scalars, time_layout, depth_layout, block_bytes, file_size, spent_bytes=0, held=())`** — `held` is charged into `structures_bytes`; `read_band_units` now passes **`(electrodes, unit_electrodes, descriptions, provenance)`** — four, not three.
...
### 4.1 The whole-suite invariant — the thing to protect

`test_measure_host_drift.py` now carries a module-level `READERS` list and a `distinct_bytes(url)` helper; `LocalFile` records every `(start, end)` range it touches and `BlockLocalFile` records each block it fetches. **`run_case` clears `READERS` before the run and then requires, on every case that reaches a record, that `distinct_bytes(processed_path) <= plan["cache_bound_bytes"]`, and raises if it matched no reader at all.** Mutation **`F1d`** sets `spent_bytes=0` — byte-for-byte the state the post-ceiling read created — and it is this invariant, not any provenance case, that catches it. **Do not weaken this into a per-case assertion.**
...
**Two things to watch on the first real read.** Resolving the chunk map costs one `get_chunk_info_by_coord` per touched chunk — on a real processed asset that is thousands of lookups, and `--plan-only` is where the cost shows up. **And since Session 30 the provenance read is part of preflight**, so `spent_bytes` on a real asset now includes it; if IBL's `general/source_script` is large, `--plan-only`'s bound will visibly include it. Both are measurements to report, not reasons to revert.
...
2. **Read the column, do not count it** (S5). **S16–S22: six sessions where the thing needed was already on disk. S23: already *computed*. S24: already *stated*. S29: already *readable*. S30 is the same lesson at one remove — `get_storage_size()` answers for a fixed-length string and lies for a variable-length one, so "readable" has to be checked per case, not assumed from an API's existence.**
3. **A check can be wrong pessimistically, and that is not the safe direction** (S5). **S17: for a host gate, pessimistic *is* the safe direction. S28: when the safe bound is loose, ask what a wrong refusal costs. S30 is the sharpest case yet and it runs the other way: an invariant that fires falsely on eighteen good cases is not "safely strict" — it would have sent me to repair correct code, and a false alarm that big is indistinguishable from a broken test.**
...
6. **Verify a write to an append-only file by reading it back** (S6). Use `io.open(..., encoding='utf-8', newline='')` from Python, never PowerShell `Add-Content`. **A heredoc through the Bash tool mangles nested quotes *and backslash escapes*** — write the script with the Write tool and run it. **`$VAR` does not expand inside the Bash tool's `-c` string passed to python**; pass the path as `argv`. **`.\venv\Scripts\python.exe` does not survive the Bash tool either — use `./venv/Scripts/python.exe`.** **S30: a heredoc that `print`s a non-ASCII character dies on cp1252 — write to a UTF-8 file and `cat` it.**
7. **Removing an unverified claim can create a new one** (S7). **S26: a repair can create a *new false claim* by describing a real measurement in words wider than the measurement. S29–S30: and a repair can create a new *false check*.**
8. **A measurement you just made is not a threshold you get to set** (S7, S17). **S27 made it mechanical: if the threshold cannot be typed, it cannot be chosen after the values are in. S30 applied it forward: `PROVENANCE_MAX_BYTES` is a module constant precisely so a candidate's own value cannot choose the number that admits it.**
...
10. **Verify a name before trusting it** (S7). **S22–S23: run the probe they hand you. S24: re-run the *proof*. S26: rebuild the *counterexample*. S28–S30: run the whole probe against the unchanged candidate before editing — seven of seven, four of four, then the Round-3 probe to the byte.**
11. **Two numbers in the same unit are not the same quantity** (S8, S11, S28, S29). **S30 is the operational form and it cost eighteen false failures: `io["bytes"]` counts every read including re-reads; `cache_bound_bytes` bounds *distinct* bytes. Before comparing two counters, ask what each one counts twice.**
12. **When a safety check fires, measure it before loosening it** (S8). **S19 inverse: when a cost is cited for *not* doing something, measure the cost. S30 second inverse: when a *new* check fires on everything at once, suspect the check — and the tell is a number that lands exactly on a structural constant, here the fixture's own file size.**
...
16. **An audit must use the same key its lookup uses** (S9, S27, S28).
17. **A pessimistic bug announces itself; a silent one does not** (S9).
18. **Read the licence before designing around it** (S9).
19. **A repair's *reason* is a general claim — go and check where else it applies** (S10). Thirteen for thirteen. **S30 is the strongest instance: the reviewer's finding was one call, and the general claim behind it — *no read after the ceiling except what the plan bounds* — became a test applied to every fixture. Fixing the call takes four lines; generalizing it is what makes the defect class fail a test instead of needing a reviewer.**
...
25. **When a test fails, first ask whether the test or the artifact is broken** (S11). **`apply_gate` returns `passed`. `measure_band_drift` returns `delta_window`. Read the parser. S30: and when eighteen tests fail at once on a check you just wrote, that question has an obvious answer and it is still worth asking out loud.**
26. **Render the output; do not read the source and assume you know what it prints** (S12). **Read the finished section back as a reviewer** (S16, S18–S30). **Eleven consecutive sessions where the read-back pass produced the last correction.**
...
46. **State a check's *resolution*, not only its role** (S21, S22, S26, S28). **S29: before writing "out of scope", check whether the thing is measurable. S30 adds the second half I had not run: check what is still *read* after the number is enforced. A bound's scope is not only what it counts — it is where it stops.**
...
57. **A check that cannot fail is not a check** (S27, S28). **S29: name checks `family/single_token`, because `re.findall(r"^  FAILED (\S+)")` captures one token. S30 made it structural: the whole-suite invariant *raises* when it matches no reader, so an invariant that silently checks nothing is itself a failure.**
58. **Method notes for the Review Method Change chat.** S26–S29 posted eight between them. **S30 posted three: that closing a review unapproved is not the same as stopping work, because clause 4 moves the repair out of the argument; that generalizing the reviewer's finding produced far more than repairing it; and — the one for Randy to watch — that clause 5 creates an incentive to narrow a successor card's scope so fewer findings can land in it, which is why RC-003's Round 1 is stated as a full pass.**
59. **A mutation that is platform-conditional must say so where it is counted** (S29).
60. **The first Convergence Decision cost one message each and closed the same session it opened** (S30). Freezing a state is not stopping work — it is what let the repair, its new coverage and the successor card all land in the session that closed the card.
...
**Session 30 readings: 09:06 PDT — 0.49 GiB free *physical* RAM, 7,067 MiB *available* including standby, VRAM 1,078 of 16,311 MiB. 09:26 PDT, with a mutation harness running — 2,521 MiB available, 41 MiB free physical, and committed memory at 128,194 of 129,385 MB**, the tightest commit reading recorded here; no single process was large (largest working set 182 MB). **Read all three numbers, not one:** `FreePhysicalMemory` excludes reclaimable standby and can read near zero while `\Memory\Available MBytes` reads gigabytes, and **committed bytes against the commit limit is the one that decides whether a new process can start at all.** I ran the two ~4-minute mutation harnesses one at a time and held the light checks until each finished. **Do not inherit these; take your own.**

**Venv:** `h5py==3.16.0`, `numpy==2.5.2` (both BSD-3-Clause), pinned in the project-root `requirements.txt` **and in the packet's own**. SpikeInterface, PyTorch and Kilosort4 are **still not installed** — that is Codex's Rung 0, and the numpy pin may have to move (change it in **both** files). **The pinned permutation stream is `numpy.random.PCG64`; a numpy change is a replay risk and the drift result must be re-replayed after one.** **h5py's chunk-index and property-list APIs are load-bearing** — `get_chunk_info_by_coord`, `get_access_plist().get_chunk_cache()`, and since Session 30 `check_string_dtype` plus `id.get_storage_size()`; an h5py change is a transfer-bound risk. Use `./venv/Scripts/python.exe` and `./venv/Scripts/pip.exe`; never bare `python` or `pip`.
...
- **The root README's running log ends at a `---` before `## What this repository will contain`. Append new entries before that separator**, and assert afterwards that the entry landed inside the log section. **58 entries; banner at 2026-08-15.**
...
- **Scripts must not print non-ASCII.** This console's stdout is cp1252. **Check the printed surface by capturing `--help`, not only by scanning source.**
- **Line endings are pinned by `.gitattributes`.** **`agents/Claude/README.md` is CRLF and must stay CRLF** (165/165 as of Session 30); the root `README.md`, the selection document and all chat files are LF. **A pattern edit over a CRLF file must preserve `\r\n` in the match string**, and assert CR == LF afterwards.
...
- **Read the parser before inventing a flag.** `test_band_drift.py` takes `--permutations`; `probe_band_drift_claims.py` takes `--module`; `test_measure_host_drift.py` takes `--keep` and `--tmp-root`; **`mutate_rc002_repairs.py` requires `--repo-root`**; Codex's probes take `--repo-root` (`probe_rc001_round1.py`, `probe_rc002_round2.py` and `probe_rc002_round3.py` require it).
