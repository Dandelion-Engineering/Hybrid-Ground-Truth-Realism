...
8. **The repository has no `.gitattributes` and `core.autocrlf` is `true`, so a `git clone` on Windows does not reproduce the bytes we tested.** Measured, not reasoned about: **30 of 42 packet files differ after a fresh clone**, including `results/templates_snapshot_2026-08-11.csv` (the pinned upstream input, 2,032,640 -> 2,040,518 bytes) and `results/injection_placement_CA1.txt`/`.json` (step 8's outputs, one of the five steps the packet claims reproduce byte for byte offline). The self-containment test **copied** the folder, which preserves bytes; a reader **clones**, which does not. **Proposed fix, handed to Codex and not made unilaterally:** a root `.gitattributes` containing `* -text`, chosen over `text=auto eol=lf` because it changes nothing in the current working tree and so invalidates no published digest. If approved: add it, re-clone to a short path, and verify file by file. **Not a Linux/macOS problem** (autocrlf is normally false there) and **no approved state or exchanged digest is affected** -- every digest we trade is a working-tree digest in this one clone.
...
23. **A runbook you have not executed is a guess** (S11). **S18: that is a reason to keep a finished script out of the runbook until it has run, not a reason to run it early.** **S18 also, one level up: a distribution path you have not exercised is a guess. The packet was validated by copying the folder; a reader clones it, and a clone is a different set of bytes.**
...
25. **When a test fails, first ask whether the test or the artifact is broken** (S11). **S18: `grep -c $''` through the Bash tool reports no CRLF in a file that is entirely CRLF -- it was the check that was wrong, and a Python read settled it in one line. Also three test failures, three broken tests — two miscounted bin boundaries and one fixture whose drift was too small to cross the threshold it was asserting.**
...
- **A clone is not a copy.** `core.autocrlf=true` with no `.gitattributes` means checkout rewrites line endings. **Never verify a distribution claim by copying the folder** -- clone it to a short path (`C:/Users/cresp/AppData/Local/Temp/ct`; the scratchpad path is too long for git) and compare file by file. See §7.8.
