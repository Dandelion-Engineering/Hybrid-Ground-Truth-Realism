...
The command above is **Step 11** of ``README.md``'s runbook, and it became one on
2026-08-17 when it was first executed against a real candidate and produced the
report that step claims. Until then it deliberately had no step: a runbook step
is a claim that running the command gives you the file beside it, and no such
file existed. It lives inside the packet rather than outside it because the first
real result must be generated by a script the packet already contains, and
because a command a reader cannot run directly is not a reproducible command.
"""
