...
no memory. One biological correction is layered on top --- a \emph{refractory period}, the brief
window after firing during which a real neuron cannot fire again
(\href{https://spikeinterface.readthedocs.io/en/stable/modules/generation.html}{SpikeInterface
generation documentation}). That explains an absence you will meet in the contract:
refractoriness is realism the pipeline \emph{already implements}, so it sits in the control arm
rather than among the three axes this project varies. What is left is the simplest possible
model of when a neuron fires, and --- as Section 3.2 argues --- where most of the missing
realism lives.
