...
That density is the good news and the reason the problem got hard. Because a neuron is
heard on several neighbouring sites at different strengths, the \emph{pattern} across sites
is a spatial fingerprint --- a signature of where the cell sits relative to the probe. That
fingerprint, averaged over many of a neuron's spikes, is called its \textbf{template}.
Templates are the central object of this project: they are what gets injected, and their
realism is what Tier A of the experiment manipulates.
...
data by \emph{relocating a real unit} --- moving an actual neuron's actual spikes to a new
position on the probe
(\href{https://doi.org/10.1007/s12021-020-09474-8}{Wouters, Kloosterman \& Bertrand, 2021,
\emph{Neuroinformatics}}). That approach carries the real spike train and the real
spike-to-spike variability along with it. The average-template-plus-Poisson approach does
not. \emph{The field has already implicitly disagreed with itself about whether this
matters}, which is a decent sign that the question is worth an experiment.
...
Because the library already carries a region label, fixing this requires \emph{no new code}
--- it is a change to which rows you select. Tier A is pure measurement, and it is first
...
The second is sharper. \textbf{Tier A is the axis most likely to move absolute scores and
least likely to move the comparison between sorters.} Changing static waveform shape changes
what every sorter's front end sees, in broadly similar ways. The temporal tiers hit collision
handling --- where, per Section 2.2, sorter families actually diverge. Since Tier A runs first
and is cheapest, there is a live risk of appearing to have answered the headline question
when the project has answered its easiest version. The contract therefore states that
\emph{the project may not conclude on Tier A alone}. That commitment exists to stop a
convenient early stopping point from becoming the result.
...
Read it aloud: \emph{how much more did realism hurt sorter 1 than sorter 2?} If $I$ is near
zero, realism moved both sorters together and the comparison between them is stable --- the
benchmark is doing its job. If $I$ is large, the gap between the two sorters depends on how
...
\emph{fraction} of the comparison that actually exists --- moving half the gap between two
sorters is enough to turn a difference a reader would call decisive into one they would call
marginal. The five-point floor prevents an absurdity: if two sorters were nearly tied in the
control arm, $0.5 \times |G_0|$ would be almost zero, and a trivial wobble would qualify as
decision-relevant.
...
are not ten independent experiments --- they share a recording, a noise environment, a random
seed. Treating them as independent would produce an interval far too narrow and a confidence
far too high. So this project resamples at the level of \emph{randomization blocks}, and it
carries $G_0$, $T$, and the final margin through every resample, so that uncertainty in the
threshold is honestly included rather than treated as if the threshold were known exactly.

Which produces three possible verdicts, not two:

\begin{itemize}
\item \textbf{Bounded positive} --- the interaction is confidently larger than the margin.
Realism changes the comparison between sorters.
\item \textbf{Bounded negative} --- the interaction is confidently smaller than the margin.
...
anatomical injection zone and matches donors to the local label there. This session added a
consequence that had been left implicit: the same host should be used across all three tiers,
because the project compares tiers to each other, and a host that changes between tiers turns
every cross-tier statement into a comparison across recordings as well as across axes.
...
This constraint is what makes CA1 the leading candidate host zone --- it is the one place where
Tier C's biological prior and Tier A's donor-availability requirement currently overlap.
...
Does the \textbf{manipulation check} pass, and at what magnitude? A weak pass is not a pass ---
it bounds every conclusion that follows.
...
One closing note on posture. Almost every safeguard described here --- the gate before sorting,
the negative control, the thresholds fixed in advance, the refusal to conclude on the cheapest
axis, the commitment to call a wide interval inconclusive --- exists to make it harder for this
project to produce a tidy answer. That is deliberate. The question is only worth asking if the
answer can be trusted afterwards, and the ways a study like this goes wrong are mostly ways it
goes wrong \emph{convincingly}.
