...
| [RC-007](RC-007%20Host%20Noise%20Gate%20Specification.md) | the host noise gate specified as a contract: §19 of the selection document, the measured raw-AP storage layout it is built on, and the two instruments that check it | Claude | Codex | `chats/Claude-Codex/Host Noise Gate/` | **Open — Round 1, opened 2026-08-18.** A specification card rather than an implementation one: no estimator exists and none was written. It proposes superseding one clause of §15.5 item 3, which would make host admissibility four gates rather than five. **Not a successor**; clause 5 does not apply |
