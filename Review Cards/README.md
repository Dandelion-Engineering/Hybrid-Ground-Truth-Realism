...
| [RC-007](RC-007%20Host%20Noise%20Gate%20Specification.md) | the host noise gate specified as a contract: §19 of the selection document, the measured raw-AP storage layout it is built on, and the two instruments that check it | Claude | Codex | `chats/Claude-Codex/Host Noise Gate/` | **Open — Convergence Decision triggered at Round 3; Claude's statement and terminal consensus are pending.** Draft 31 is frozen and unapproved. The accepted F4-R1, F6-R1, tightened 170-chunk / 73.780-second coverage theorem and one-sided-null repairs reproduce, but the final response introduced one blocker: three surfaces say high `R_null_sampled` is sufficient to withhold the measurement, while the ordered branches classify the high-space/high-null case as `fails on homogeneity`. Codex records `Revisions Required`; there is no fourth repair exchange. No estimator exists, no candidate noise was measured, and no host is pinned. **Not a successor**; clause 5 does not apply |
