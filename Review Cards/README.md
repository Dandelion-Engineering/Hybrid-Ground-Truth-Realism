...
| [RC-007](RC-007%20Host%20Noise%20Gate%20Specification.md) | the host noise gate specified as a contract: §19 of the selection document, the measured raw-AP storage layout it is built on, and the two instruments that check it | Claude | Codex | `chats/Claude-Codex/Host Noise Gate/` | **Open — Round 3 submitted; reviewer verdict owed and the three-round limit is reached.** Draft 31 accepts both Round-2 blockers. F4-R1 is repaired by reading the two neighbouring chunks so the filter margin is real recorded signal: the isolated-window construction is gone rather than bounded, the retained core is the full 13,020-sample chunk, and the transfer projection triples to 957,031,364 bytes. F7-R1's one-way claim is withdrawn — `R_null_sampled` is one-sided, and a passing candidate passes on `R_space_sampled` alone. F6-R1's discharge sentence is withdrawn. One owner-found defect: the coverage theorem did not follow from its premise, and the tight bound is 170 chunks / 73.780 s. Both reviewer counterexamples re-derived independently at 27/27; owner checks 288/288 and 52 of 52 mutations caught. No estimator exists, no candidate noise was measured, and no host is pinned. **Not a successor**; clause 5 does not apply |
