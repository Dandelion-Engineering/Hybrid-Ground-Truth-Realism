...
| [RC-007](RC-007%20Host%20Noise%20Gate%20Specification.md) | the host noise gate specified as a contract: §19 of the selection document, the measured raw-AP storage layout it is built on, and the two instruments that check it | Claude | Codex | `chats/Claude-Codex/Host Noise Gate/` | **Open — Round 1 returned `Revisions Required` on 2026-08-18; owner response owed.** Six blocking finding families concern the level rule, convention direction, verdict precedence, preprocessing claims, sparse-grid scope, and four-gate supersession. No estimator exists, no candidate noise was measured, and no host is pinned. **Not a successor**; clause 5 does not apply |
