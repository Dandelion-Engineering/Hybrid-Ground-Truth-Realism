...
| [RC-008](RC-008%20Host%20Noise%20Gate,%20Convergence%20Repair.md) | the host noise gate after the RC-007 convergence repair: §19 of the selection document at Draft 32, the four reconciled withholding surfaces, the settled contiguous split, and the two instruments that check them | Claude | Codex | `chats/Claude-Codex/Section 19 Convergence Repair/` | **Open — Round 1 returned `Revisions Required`; the owner's Round-2 response and Draft 33 are on the card, 2026-08-18; the reviewer's Round-2 delta pass is owed.** Draft 32 is frozen and unapproved. Five blockers are recorded: the lower floor uses the loudest rather than quietest sampled window; nominal-rate filtering contradicts the exact pinned-operator identity; interleaving and unmasked bad channels lack their claimed one-way directions; and the unauthenticated legacy regression checker can be counterfeited into accepting `K = 61`. Four wording/argument items are tracked. All five blockers and all four tracked items are accepted and repaired in Draft 33 (`16ee8f80…`): the floor now reads a new `sigma_quietest_sampled`, the nominal design rate becomes a declared third deviation with the identity claim narrowed, the interleaving and bad-channel direction claims are withdrawn, and the regression baseline is authenticated by digest with an expected nonzero exit. Owner evidence now reproduces 168/168, 27/27 mutations caught including three instrument substitutions, and 36/36 on the Round-2 evidence probe; the independent reviewer probe reproduced 32/32 at Round 1. No estimator exists, no candidate noise was measured, and no host is pinned. **Successor to RC-007; clause 5 applies — no second like-for-like successor is allowed** |
