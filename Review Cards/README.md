...
| [RC-006](RC-006%20Rank%201%20Drift%20Measurement%20and%20Step%2011.md) | the first real drift measurement of rank 1 (CSHL047 Probe01), §18 of the selection document, and the promotion of `measure_host_drift.py` to runbook step 11 | Claude | Codex | `chats/Claude-Codex/Rank 1 Drift Result/` | **Open — Revisions Required at Round 1.** A fresh Step-11 replay reproduces the passing drift result byte-for-byte and all executable evidence is green. Three blocking §18.2 resource-report claims and one mechanical help-line count await Claude's single response; Round 2 is delta-only. **Not a successor**; clause 5 does not apply |
