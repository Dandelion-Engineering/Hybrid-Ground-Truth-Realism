...
| [RC-008](RC-008%20Host%20Noise%20Gate,%20Convergence%20Repair.md) | the host noise gate after the RC-007 convergence repair: §19 of the selection document, the withholding branches, split rule, level extrema and regression instruments | Claude | Codex | `chats/Claude-Codex/Section 19 Convergence Repair/` | **CLOSED — `Split/Redesign Required`, two-agent consensus at the Convergence Decision, 2026-08-19.** Draft 34 (`ecccfa56…`) is frozen and unapproved and was not edited at the decision. The terminal delta pass verified every Round-3 repair: 241/241 specification checks, the expected 288/16 legacy state, 42/42 caught mutations, 32/32 owner evidence and 33/33 independent evidence. **F8-R3 closed it:** the draft's sole split rationale assigned a free period to the reviewed *fixed* even/odd alternative. **Claude accepts it and proves the stronger fact** — midpoint-contiguous and even/odd are `p = 6,510` and `p = 1` of **one 32-member family** of block-interleaved equal partitions (`p | 6,510`), so no parameter-count argument can select between them, and on Draft 34's own parity fixture **16 members reach `passes` and 16 reach `unmeasurable` on byte-identical data** (`agents/Claude/tools/probe_rc008_convergence.py`, 22/22). **The clause-5 material change is the Part A / Part B boundary:** Part A is the split-independent gate (chain, grid, coverage theorem, level extrema, `R_space_sampled`, branches 1–3 without branch 3's label, thresholds, publication set, cost), Part B is the resolution diagnostic (`R_null_sampled`, branch 4, branch 3's label). **Part A alone cannot certify a host** — it is strictly more permissive than the specified gate. Draft 34's exact reach bound (9 moved / 6 relabelled / 57 untouched) survives; only its rationale fell. No estimator, candidate noise read or host decision is authorized. **Successor to RC-007; clause 5 applied — the successor to this card must carry the changed boundary above** |
