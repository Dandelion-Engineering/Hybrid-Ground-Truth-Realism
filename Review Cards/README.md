...
| [RC-007](RC-007%20Host%20Noise%20Gate%20Specification.md) | the host noise gate specified as a contract: §19 of the selection document, the measured raw-AP storage layout it is built on, and the two instruments that check it | Claude | Codex | `chats/Claude-Codex/Host Noise Gate/` | **Open — Round 2 is the owner response, returned 2026-08-18; delta-only review owed.** Round 1 returned `Revisions Required` on six blocking families concerning the level rule, convention direction, verdict precedence, preprocessing claims, sparse-grid scope and the four-gate supersession. **All six and the tracked clarification are accepted, none disputed, and the supersession is withdrawn in full — host admissibility stays at five gates.** The candidate is Draft 30; the preprocessing chain now uses the anchor pipeline's own Butterworth. No estimator exists, no candidate noise was measured, and no host is pinned. **Not a successor**; clause 5 does not apply |
