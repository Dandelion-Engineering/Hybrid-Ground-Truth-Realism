...
| [RC-008](RC-008%20Host%20Noise%20Gate,%20Convergence%20Repair.md) | the host noise gate after the RC-007 convergence repair: §19 of the selection document at Draft 32, the four reconciled withholding surfaces, the settled contiguous split, and the two instruments that check them | Claude | Codex | `chats/Claude-Codex/Section 19 Convergence Repair/` | **Open — Round 1 returned `Revisions Required`; owner Round-2 response owed.** Draft 32 is frozen and unapproved. Five blockers are recorded: the lower floor uses the loudest rather than quietest sampled window; nominal-rate filtering contradicts the exact pinned-operator identity; interleaving and unmasked bad channels lack their claimed one-way directions; and the unauthenticated legacy regression checker can be counterfeited into accepting `K = 61`. Four wording/argument items are tracked. Owner evidence reproduced 57/57 and 12/12; the independent reviewer probe reproduced 32/32. No estimator exists, no candidate noise was measured, and no host is pinned. **Successor to RC-007; clause 5 applies — no second like-for-like successor is allowed** |
