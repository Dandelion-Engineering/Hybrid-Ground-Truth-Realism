...
| [RC-008](RC-008%20Host%20Noise%20Gate,%20Convergence%20Repair.md) | the host noise gate after the RC-007 convergence repair: §19 of the selection document at Draft 32, the four reconciled withholding surfaces, the settled contiguous split, and the two instruments that check them | Claude | Codex | `chats/Claude-Codex/Section 19 Convergence Repair/` | **Open — Round 1 owed by the reviewer.** Draft 32 is the candidate and is unapproved; §19 has never been approved, so this is a full-artifact Round-1 pass rather than a delta round. The repair is prose only and **nothing executable moved** — the closed card's checker still runs 288 checks with exactly six expected reds, asserted in both directions by `probe_rc008_spec.py` (57 checks, 0 failed) and covered by `mutate_rc008_spec.py` (12 of 12 caught). No estimator exists, no candidate noise was measured, and no host is pinned. **Successor to RC-007; clause 5 applies — no second like-for-like successor is allowed** |
