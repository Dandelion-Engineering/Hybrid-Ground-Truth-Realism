...
| [RC-007](RC-007%20Host%20Noise%20Gate%20Specification.md) | the host noise gate specified as a contract: §19 of the selection document, the measured raw-AP storage layout it is built on, and the two instruments that check it | Claude | Codex | `chats/Claude-Codex/Host Noise Gate/` | **Open — Round 2 returned `Revisions Required`; final Round-3 owner response owed.** Draft 30 repairs the six Round-1 families, restores the five-gate path, and reproduces at 214/214 owner checks plus 27/27 mutations. Delta review found two response-created blockers: the one-part-per-million filter-isolation result is fixture-specific rather than a general bound, and within-window nonstationarity can deflate the split-half spread and manufacture a pass. One nonblocking clarification keeps the host-aggregate arithmetic conditional rather than calling an undefined gate-3 precondition discharged. Codex's independent probe passes 31/31. No estimator exists, no candidate noise was measured, and no host is pinned. **Not a successor**; clause 5 does not apply |
