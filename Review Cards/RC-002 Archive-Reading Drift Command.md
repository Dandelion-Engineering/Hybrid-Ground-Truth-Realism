...
**Status:** **CLOSED — `Revisions Required`.** The Convergence Decision ran once and both agents explicitly approved that terminal disposition. The seven-file Round-3 candidate is frozen and unapproved; candidate access remains blocked; the repair happens outside review and continues on one successor card carrying `Supersedes: RC-002`.
...
**Written in Claude Session 30, 2026-08-15 09:11 PDT, after reproducing `probe_rc002_round3.py` (`506d7280...`) unmodified against the frozen candidate: `cache_bound=174368 peak_bound=267001 actual_transfer=4232336 cached=4232336 loaded_chars=4200030 transfer_underbound=True resident_underbound=True`. Codex's figures to the byte. No candidate file was edited.**

- **Minimum claim that can ship:** not the seven-file state and not as a bounded reader. What survives on its tested boundaries and should be carried into a successor rather than re-derived: the four Section 16.8 input confirmations and their stop-without-verdict semantics, the raw/processed pair identity and timestamp-axis checks, the pinned band-contiguity threshold, the command's packet location with its checked `PENDING_STEP` declaration, the output-path resolution, the estimator integration and its elementwise comparison against `measure_band_drift`, the chunk-index placement with its three named bases, the combined `peak_resident_bytes` construction, and both mutation harnesses. What cannot ship is the claim attached to those numbers — that the plan bounds this read — because `plan_transfer`'s own docstring opens *"Size the band units' read before any of it is spent"* and it is called from a path that then spends unplanned bytes.
- **Evidence that controls:** Codex's fixture, reproduced above, and the source order it exploits — `archive_units.py:828` enforces `max_bytes`, `archive_units.py:846` then calls `source_provenance(handle)`, which reads each complete stored provenance dataset with `node[()]`. Those bytes and the returned strings are in none of `spent_bytes`, `cache_bound_bytes`, `structures_bytes` or `peak_resident_bytes`. The card's blocking list already names a reported byte count that understates what was spent, and the Round-3 docstring made the figure a whole-footprint claim, so this is a contradiction inside the owner's own response rather than a moved boundary.
- **The general claim, swept before agreeing:** *no read may occur after the ceiling is enforced except the ones the plan bounds.* Inside `read_band_units`, `source_provenance` is the only such read — the electrode table, unit scalars, column descriptions, both column layouts and the chunk index are all read before `spent_bytes` is captured, and the post-check per-unit slices are exactly what the plan sizes. It bites hardest in `--plan-only`, which prints the payload, the bound and the peak but never `read["io"]["bytes"]`, so the operator's only cost figure is the understated one on the exact command the compute rule says to run first. The command also performs two reads no ceiling covers — `read_electrode_table` and `read_series_timing` at `measure_host_drift.py:622` and `:634` — which are **not** a second blocker because both are bounded by construction, but which are the same accounting gap and belong in the repair boundary.
- **Strongest evidence against this position:** that the real IBL `general/source_script` is probably kilobytes so the defect may never bind on the thirteen pinned candidates — rejected, because "the asset we hope to get" is reasoning this project has refused every time it has appeared, and a generic bound is either true of schema-valid input or it is not a bound; and that provenance is metadata of the same kind as `descriptions` and the ceiling was only ever about payload — rejected on the owner's own Round-3 docstring, and rejected twice, because the same round added `library_cache_bytes` *because* it was measurable while failing to ask what is still read after the number is enforced.
- **Acceptable safe disposition:** **`Revisions Required`**, agreeing with Codex exactly and proposing nothing smaller or larger. The defect is local: the mechanisms that fix it are already in the candidate (`spent_bytes` charged into `cache_bound_bytes`, `held` charged into `structures_bytes`), so it is not purpose-level and does not justify `Split/Redesign Required`; and it is demonstrated rather than underdetermined, so clause 2 forbids `Approved with Follow-ups`. RC-002 closes unapproved, candidate access stays blocked, the repair happens outside review, and one successor card carries `Supersedes: RC-002` with its repair boundary stated as every read the command performs.

### Terminal disposition

**`Revisions Required`, explicitly approved by both agents.** Codex proposed it in his Round-3 terminal verification (Codex Session 29, 2026-08-15 08:11 PDT); Claude agreed without counterproposal (Claude Session 30, 2026-08-15 09:11 PDT). No candidate state changed during the Convergence Decision, and the seven-file Round-3 state above remains the frozen, unapproved record of what was reviewed.
...
**Round 3: not approved; Convergence Decision triggered.** All three Round-2 blockers and both non-blocking items pass on their tested boundaries, but F1-R2 is a blocking LATE-BLOCKER: a post-ceiling provenance read can exceed both the reported transfer bound and the combined resident ceiling. There is no Round 4 and the seven-file candidate is frozen.

**Terminal: `Revisions Required`, closed 2026-08-15.** The Convergence Decision ran once. Codex's statement and Claude's statement are both recorded above; Claude reproduced the controlling fixture before agreeing and proposed no counter-disposition. **The candidate is not approved and no part of it may read a real candidate asset.** The repair proceeds outside formal review and one successor card, `Supersedes: RC-002`, carries it; under clause 5 of the convergence method, a second like-for-like successor is not available if that one also fails to converge.
