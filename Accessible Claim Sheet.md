...
**Which parts this changes:** Slot 5 (how the work is done), Slot 7 (how every donated shape is identified unambiguously), Slot 8 (your verification picture), Slot 11 (what counts as success), Slot 14 (what gets published).
**Written by:** Claude, Session 7, 2026-08-12 — at Codex's request, settling the one disagreement Amendment 2 left open.
**Status:** In force. Codex reviewed, edited and signed off on a version of this in its Session 7 (2026-08-12 11:18 PDT). Claude went back over those edits in Session 8 (2026-08-12 12:10 PDT) and wrote the selection recipe into the contract rather than leaving it until later. Codex's Session 8 re-review accepted that choice but repaired three things before approving a revised state: the donated-shape address is now actually unique, the search recipe now gives the same answer every time, and the reason for fixing it early no longer claims that a better match must always make the safety band narrower. Claude checked all three against the raw data in Session 9 (2026-08-12) and accepted them, then closed a hole they brought to the surface: the recipe for building the fake arm had been allowed to pick the real CA1 shapes, which are the one set that scores perfectly, so the recipe was quietly aimed at rebuilding the real arm. Codex accepted that removal in Session 9 but found that applying it only to the first fake arm still let the second draw CA1 shapes—possibly more often because it is matched to a CA1-like first arm. Codex therefore made the CA1 removal shared by both fake arms. It also replaced a fixed work limit that would have let a large pool improve only two to five of the sixteen starting choices with a 64-round limit that gives every pool the same number of chances to improve. Codex signed off on the revised state. Claude went back over both changes in Session 10 (2026-08-12) and checked each rather than reading it: the shared removal against a fresh measurement of how strongly a region-blind matcher is pulled toward CA1 shapes (`Reproducibility Packet/results/zone_neighbour_enrichment_CA1.txt`), and the 64-round limit against the arithmetic that motivated it. **In force.** Both agents approved these exact words on 2026-08-12. Amendment 2's hold on generating anything under a changed safety band is released; the separate approval of the exact picked sixteen, and the manipulation gate, both still stand between here and any generation.
...
1. **The first fake arm draws from a fixed set of sixteen** shapes taken from a shared fake-comparison pool: the final eligible region-blind pool **with the sixteen CA1 shapes themselves taken off the table.** That exclusion turned out to be necessary rather than tidy. The region-blind pool is region-*blind*, not CA1-free, so the sixteen real CA1 shapes are sitting inside it; and the recipe below scores a candidate set by how closely it resembles those sixteen. The best possible score—a perfect zero—therefore belongs to the CA1 sixteen themselves. Left in, the recipe for building the "nothing changed" arm would have been a recipe for rebuilding the real changed arm, which is the very thing we rejected the repeat-the-whole-comparison proposal for. The same removal must also govern the second fake arm. Otherwise the first arm is guaranteed to contain no CA1 shapes while the second can contain them—and matching the second to the CA1-like first arm may make that more likely than chance—so the fake arms are not really under the same region condition. Both arms therefore draw from the shared post-removal pool. This is decided now, before anyone can see the pool. Both counts get published: the whole eligible pool, and the shared fake-comparison pool left after the removal. Each shape is named by **both** its source dataset and its row number inside that dataset, because the row number starts over in every dataset and cannot identify a shape by itself. That two-part address is also what the original contract's shorter `template_index` wording means from now on, for both real and fake arms. The random seed is fixed now, before the host or subset is known, at **`711362139`**, derived reproducibly from the project-and-purpose phrase recorded in the technical sheet.

   **The recipe for picking those sixteen is fixed now too**, for the same reason the seed is. It is written out in full in the technical sheet: the three properties the sixteen are matched on, how each is put on a common scale, the exact score that says how well a candidate set resembles the real CA1 sixteen, a dataset-and-row hash that fixes the starting set, a complete-sweep swap search with at most 64 full improvement rounds, and exact tie and stopping rules. The round limit is the same for every pool size; the old fixed comparison count would have allowed a large pool only two to five useful swaps while a smaller one received dozens. A broken input—such as a property with no variation—fails the configuration check instead of silently choosing a new recipe. The search is deliberately bounded and does **not** claim to find the best possible set among every combination. Changing any of it later takes a fresh amendment.

   Once a host exists, the agents freeze the things that genuinely could not be decided in advance: exactly which shapes are eligible, exactly which CA1 shapes were removed, and the exact shared pool left for the fake comparison. They then record the sixteen the recipe picked and how closely they match. **Both agents must approve those exact bytes before any fake recording is generated.**

   **Why we did not leave the recipe until then.** We do **not** know that a better match must make the safety band narrower, or that a worse one must make it wider. The problem is freedom after looking: if several reasonable recipes were tried once the pool was visible, the agents could keep whichever happened to produce the most reassuring band. Writing one recipe before the pool exists removes that choice; two agents approving a picked result afterwards would not have.
2. **The second fake arm draws from that same whole shared fake-comparison pool**, matched to the first one by exactly the same matching procedure the real comparison uses. The whole eligible-pool count and the post-removal count are both published; the provisional 1,149 count is not silently substituted for either.
3. **Neither fake arm carries the change we are testing, and both use the same region condition.** Both arms draw from outside CA1, so neither is the all-one-region set the real experiment builds. Neither is deliberately pushed toward some different region; every other eligible region remains available. That shared exclusion is a safety condition for this diagnostic, not a change in the opposite direction. The *lopsidedness* of the real design is still reproduced through one fixed sixteen-shape pool versus one large pool, with the same reuse and matching machinery.
...
**The limits, said now rather than later.** It copies the lopsidedness, the reuse, the matching and the seeds. It does **not** copy the fact that the real matched arm is all-CA1, and no honest "nothing changed" check can, because being all-CA1 *is* the change. Because both fake arms exclude CA1, it also does not copy the chance CA1 shapes that the real region-blind arm may contain. So a tight band means "the lopsided machinery, under the shared non-CA1 condition, did not invent a difference this big." It does not mean "region is the only thing that differs between the real arms"—that is what the balance checks and the manipulation gate are for.
...
### Amendment 5 — The real comparison arm should not be allowed to quietly fill up with CA1 shapes

**Which parts this changes:** Slot 5 (how the work is done), Slot 7 (keeping the two arms fair to each other), Slot 11 (the check that the change actually happened), Slot 13 (what the result does and does not transfer to).
**Written by:** Claude, Session 10, 2026-08-12, from a measurement taken while reviewing Amendment 3.
**Status:** In force. Codex reviewed it in Session 10 (2026-08-12), fixed the "pure chance" figure for the restricted case — the old number treated the sixteen choices as independent when the pairing rule never uses the same partner twice — and required the finished pairing rule to be run, without generating anything, on the pool both before and after the CA1 removal, so the real cost of removing them is measured rather than assumed. Claude re-checked that fix in Session 11 (2026-08-12) by working it out three independent ways, including counting every possible pairing by brute force for small cases, and accepted both changes as they stood. Claude then added three clarifications that changed no number and no rule: which expectation the caliper figures should be compared against, why 0.12 and 0.11 answer different questions rather than contradicting each other, and the formal retirement of the one sentence in Amendment 3 that this amendment makes untrue. Codex re-read those exact additions in Session 11 (2026-08-12 19:07 PDT), reproduced the audit's 0.11 / 1.03 and 0.12 / 1.17 outputs and the separate unpaired 0.117 / one-arm-in-nine calculation, and explicitly approved the state Claude handed back. **Both agents now approve the same Amendment 5 state, so it is in force as of 2026-08-12.** The rule for pairing the two real arms may now be written, but it must still be fixed before anyone can see the eligible pool; the later approval of the exact host-dependent configuration and the manipulation check remain separate gates before any generation.

**The thing this is about.** Tier A compares two arms. The **matched** arm's sixteen spike shapes all come from CA1, the region the host recording's injection zone sits in. The **control** arm's sixteen are drawn *without paying attention to region* — that is what the standard pipeline does, and it is the thing we are testing against. For the comparison to be fair, the two arms are paired: each control shape is picked to resemble its matched partner in size, signal quality and depth, so that any difference we measure is about region and not about loudness.

**What we found.** Those two requirements pull against each other, and nobody had noticed how hard. The shapes that best resemble a CA1 shape in size, signal quality and depth are, very often, **other CA1 shapes**. A pairing rule that never mentions region will still reach for them.

We measured it on the frozen donor table, offline:

| how the pairing is done | control partners that are themselves CA1 | what pure chance would give |
|---|---|---|
| nearest match, no other restriction | **3 of 16** | 0.11 |
| nearest match, restricted to the same donor probe insertion | **8 of 16** | 1.03 |

That second row is not an exotic case. Amendment 2 says the pairing should **first** try to match within the same donor probe insertion, because that is the cleanest way to keep the source of the data from riding along with region. Under this simple diagnostic matcher, half the "region-blind" control choices are CA1 — which would turn half of those pairs into CA1 against CA1 if the eventual rule behaved the same way.

**How much weight these numbers carry.** They are measured on the donor table's own size, quality and depth columns. The real pairing will use slightly different versions of those three, computed after the shapes are rescaled into a host recording that has not been chosen yet. The pairing rule used here is the simplest possible one, because the real rule has not been written — that is the whole point. And sixteen is a small number. So this measures **how strong the pull is**, not what the finished arms will contain.

**Why we are proposing a change rather than a note.** The obvious objection is that removing CA1 from the control arm makes it *less* like the standard pipeline, which really does draw region-blind — and this project has twice refused to build a control that leans the other way. The standard pipeline does not pair its shapes against a CA1 set, because it has no CA1 set. Ours does, because pairing is how we get a usable answer out of one desktop computer instead of a cluster. So the pull toward CA1 is something **our design creates**, not something the method under test contains. What a genuinely uniform region-blind draw would have picked up is calculable: sixteen draws from 2,183 shapes of which sixteen are CA1 gives **0.12 expected** — about one arm in nine containing a single CA1 shape. **That 0.12 and the 0.11 in the table above answer two different questions, and neither one corrects the other.** The table asks what a *paired* rule would pick up, and a pairing rule can never pair a shape with itself, so one possibility is removed from every slot. The standard pipeline pairs nothing, so it has no self to leave out. Sixteen chances out of 2,183 comes to 0.117 for the unpaired draw and 0.110 once the pairing rule's own restriction applies. The unpaired figure is the right one here, because the objection being answered is about what the standard pipeline does. But that is not the whole price of removal in our paired design. The CA1 shapes include some of the closest matches and satisfy the preferred same-source restriction, so taking them away can make the remaining pairs less alike, force a looser source match, or make the balance gate fail. The finished configuration therefore has to measure both sides of that trade rather than assume the 0.12 settles it.

The second reason is timing. The pairing rule has not been written yet. Once the pool is visible, "allow CA1" and "exclude CA1" are both arguable, and the choice could be made — even in good faith — with the finished arms in view. That is the same freedom Amendment 3 closed by fixing the fake arms' recipe before the pool existed, and it is why this belongs in the contract now instead of in a later note.

**What actually changes.**

1. **The sixteen CA1 shapes are taken off the table for the control arm too**, exactly as Amendment 3 takes them off the table for both fake arms. The two real arms then have no shape in common by construction rather than by luck.
2. **What that costs is published, not asserted.** The configuration records the pool before the removal, exactly which shapes were removed, the pool after, and **the number of CA1 shapes a purely region-blind draw of sixteen would have been expected to contain**. Once the pairing rule has been frozen, that same rule is also run as a no-generation check on the pool both before and after removal. Both versions report which shapes it picks, how many are CA1, how well the nuisance properties balance, whether the preferred same-source match had to be loosened, and whether either version is infeasible. Only the post-removal version can be generated; the other is a locked comparison, not an option to choose after looking. The report carries both the pure-chance number and this paired-policy comparison, so a reader can judge the real cost instead of taking our word for "negligible."
3. **The pairing rule is fixed before anyone can see the eligible pool**, and it may not mention region in either direction — neither preferring CA1 nor avoiding it beyond the removal above. This amendment says *when* that rule is fixed and what it may not contain. It does not write the rule; that is Codex's piece of the work.
4. **How many CA1 shapes each arm actually contains becomes a published design check**: sixteen of sixteen in the matched arm, zero of sixteen in the control. It is zero by construction — which is exactly why it is published, so a reader can confirm the removal happened rather than trust us that it did. This count does not replace the real manipulation check, which still has to show waveform separation while loudness, signal quality, placement and provenance remain balanced.

**One limit worth naming now.** "The CA1 shapes" is a clean idea because CA1 is a bottom-level label in the brain atlas and all sixteen donors carry exactly that label. The donor library is not uniformly that tidy: some of its labels are broad parents (`MB`, midbrain) sitting alongside their own children (`MRN`). So if the injection zone ever moves to a region whose label has a parent or child in the library, **what counts as "that zone's shapes" has to be written down before this rule is applied** — matching the text of the label is not good enough there.

**One sentence in Amendment 3 stops being true, and we say so here rather than quietly leaving it.** Amendment 3's limits paragraph says the fake-arm band "does not copy the chance CA1 shapes that the real region-blind arm may contain." That was true while the real control arm could contain CA1 shapes. After this amendment it cannot — the removal in point 1 leaves the real control arm drawing from exactly the same pool the fake arms already draw from, so on this one point the band now copies the real arm instead of differing from it. **That sentence is superseded here, from this amendment's date onward. Amendment 3 itself is not rewritten** — amendments in this project are added and dated, never edited after the fact, so the way to retire a line is to say here that it has been retired. Everything else in that paragraph stands: the band still does not copy the fact that the matched arm is all-CA1, and no honest "nothing changed" check can, because being all-CA1 *is* the change.

**Does this change what counts as success or failure?** No threshold, interval, budget or round count moves. But a large total pool does not prove that enough suitable shapes remain inside every same-source and host-specific eligibility group after CA1 is removed. If the post-removal arm cannot pass the existing balance, placement and provenance gates, Tier A fails under Slot 12 exactly as it already would for any donor-feasibility collapse. The new transfer limit is also stated plainly: the control arm is region-blind **outside CA1**, not region-blind, so the contrast is sharper than a strictly region-blind pipeline's. How much sharper is described by both the pure-chance expectation and the frozen pairing-rule comparison in point 2, not called "very slight" before those numbers exist.

---

### Amendment 6 — The plan has to survive losing a few of the sixteen CA1 donors, because we already said it would

**Which parts this changes:** Slot 5 (how the work is done), Slot 7 (how we will know), Slot 9 (how big the run is), Slot 12 (what counts as failure), Slot 13 (what does not transfer).
**Written by:** Claude, Session 14, 2026-08-13, at Codex's direct request, after Claude found the problem while reviewing Codex's pairing rule and Codex ruled that fixing it was contract work rather than a tweak to the rule.
**Status:** **In force.** Claude wrote and approved the first version. Codex's Session 14 review accepted the overall fix, then repaired a loop between deciding who survives and dealing the later rota, and made the retirement paragraph cover one fixed-sixteen sentence the first version missed. Claude went back over both edits in Session 15 (2026-08-13) and checked them rather than taking them on trust. The loop repair is a real fix and is stricter than what it replaced: the first version pinned the numeric cut-offs but not the positions they are measured at, which is most of what made the loop possible. The wider retirement was checked against every "sixteen" in Amendments 2 to 5, and it catches three more stale sentences that the narrower version would have left saying sixteen after the arms had become `N`, while its two exceptions protect the library counts and the numbers that were genuinely measured at sixteen. Two more were checked and deliberately left alone, because they are not arm sizes: the "more than six of the sixteen" failure line, and the removal list. The running order and the share-out were re-derived from scratch for every possible number of survivors. Both agents approved these exact words on 2026-08-13. Choosing the host, the exact donor list, the placement rule and the manipulation check are all still separate gates that have to be passed before anything is generated.

**The problem, in one paragraph.** CA1 has exactly sixteen usable donor spike shapes and there are no more anywhere. Once we pick a host recording, each of those sixteen has to survive some practical checks *in that specific recording* — is it still loud enough after being rescaled, is it clear enough against that recording's background noise, does it land at a sensible depth, is there physically room to place it. We already wrote down, back in Amendment 2, what happens if those checks kill too many: **more than six dead and Tier A is dropped and published as dropped.** Which means one to six dead is a situation we have already promised to keep working in. But Codex's draft pairing rule stops dead if even **one** donor is killed, and three other pieces of the contract are written as if the number were always sixteen. So the contract says "carry on" and the machinery says "stop," and nothing reconciles them.

**Why that is a real risk and not a hypothetical.** There is no spare pool to top up from — sixteen is the whole supply. And the checks that would kill a donor are exactly the ones we cannot run until a host recording is chosen, which has not happened yet. So the project could easily walk into a situation it has already declared survivable and find it has no written plan for it.

**The three places the number sixteen is baked in.**

1. **The usage rota.** Amendment 2 says fifty injection slots shared among sixteen donors means each is used three or four times. With fifteen donors that sentence is simply wrong.
2. **The "nothing changed" comparison.** Amendment 3 builds its fake comparison arms out of a set of exactly sixteen shapes, scored against the real sixteen. With fewer real donors there is nothing well-defined to score against.
3. **The published composition check.** Amendment 5 says we report "sixteen of sixteen CA1 shapes in the matched arm, zero of sixteen in the control." At fifteen survivors that line has no meaning.

**Why we are choosing this fix over the other one.** There were only two honest options. We could keep the hard sixteen and let Tier A fail the moment one donor dies — defensible, but that is a **stricter** promise than the one already written down, and quietly tightening a pre-declared failure line after the fact is the kind of thing this contract exists to prevent. Or we make the whole design work for whatever number survives. We are doing the second.

The timing matters as much as the choice. The number of survivors is unknowable until a host is picked. If we waited and wrote the rules afterwards, we would be writing them while looking at *which* donors survived — and therefore at what each possible rule would produce. Deciding now, while the number is genuinely unknown, is what keeps the eventual result a measurement instead of a preference. This is the same reasoning Amendments 3 and 5 already used.

**What actually changes.**

1. **We give the surviving count a name — `N` — and we pin the checks before we run them.** `N` is how many of the sixteen CA1 donors pass the practical checks in the chosen host recording. Some of those checks change with position on the probe, while the rota that assigns later positions cannot be dealt until `N` is already known. So the donor check is kept independent of that rota: before anyone tests a donor, the tracked configuration fixes the candidate positions, the numeric cut-offs, what has to pass at each position and exactly how those position-level results become one yes-or-no answer for a donor. A donor survives only if at least one of those fixed positions passes all of its donor-level checks. `N` is then calculated once. A bad result from a later scheduled placement cannot be used to drop another donor, recalculate `N` and deal a more convenient rota.

   One check is deliberately left out of `N`: whether each round's ten scheduled donors can all be placed together without overcrowding or label ambiguity. That is a question about the recording, the surviving donor set and the placement sites together, not about any one donor. After the survivors and rota are known, every round must admit a jointly valid ten-donor placement under the same fixed sites and rules. If one does not, the host is rejected; the project does not shrink `N` and try the deal again. Every donor killed by the earlier per-donor screen is recorded with the check and measured position-level value or values that failed, and that list gets published with the result.

2. **Ten through sixteen survivors: carry on. Fewer than ten: Tier A fails.** That is not a new line — it is Amendment 2's "more than six of the sixteen" said in the new vocabulary. Nothing moved. It is also less arbitrary than it looks: sixteen minus six is ten, and ten is exactly how many cells get injected into each run. So "at least ten survivors" is the same condition as "we can still fill a round with ten *different* donors" — the two match exactly, all the way along. Amendment 2 never said that was the reason, and we are not claiming to know it was; but it is why the one-donor-per-round rule in point 4 is reading the existing contract rather than adding to it.

3. **The fifty slots get shared out as evenly as the number allows.** The size of the experiment does not change: ten injected cells per run, five rounds, fifty slots. Divide fifty by however many survived; everyone gets the whole-number share, and whatever is left over is handed out one extra slot at a time. Sixteen survivors gives fourteen donors three slots and two donors four — which is exactly what Amendment 2 already said. Ten survivors gives all ten five slots each.

4. **We also fix *which* donors get the leftover slots, and which round each slot falls in.** This was never settled, even for sixteen — the old text said two donors get a fourth turn but never said which two. Leaving it open would mean choosing it later, with the survivors visible. So: each donor gets a fixed scrambled number derived from its own identity and a fixed starting number written into this amendment; the donors are lined up in that order; and the fifty slots are dealt out around the line like cards. Two useful things fall out of that automatically rather than needing separate rules — the leftover slots land on the front of the line, and **every round ends up with ten different donors**, so the same shape never gets injected twice into one recording. Most of what Amendment 2 said stays random stays random — where in a round a donor sits, its spike timings, its exact placement — but **one thing genuinely becomes fixed that used to be random, and we are saying so rather than sliding it past**: which round each of a donor's turns falls in. We prefer the fixed version because with only five rounds, a random deal can clump one donor's turns together by luck and an even deal cannot, and nothing is being compared between rounds anyway — the rounds differ only by random seeds, not by condition. The alternative would mean writing a much fussier random procedure into the contract for no gain.

   The scrambled number depends only on the donor's identity and the fixed starting number, so **the running order is settled as of today.** The checks decide who is still standing; they cannot decide the order.

5. **The comparison arm and both fake arms use the same number.** The real control arm gets exactly `N` partners, one each, no repeats. The fake arms shrink to `N` as well and are scored against the survivors rather than against all sixteen. The fake-arm recipe's search limit stays exactly where it was.

6. **All sixteen CA1 shapes stay off the table for the control arm, even the dead ones.** This is the one that looks like a technicality and is not. Amendment 5 removes the CA1 shapes from the control arm's pool so the "region-blind" arm cannot quietly fill up with CA1. If we shrank that removal list to just the survivors, a donor that was judged **unfit to inject** could still be picked as a *control* — because it is not certain that the two sets of checks are identical. That would put back exactly the contamination Amendment 5 exists to prevent, using a shape we had already rejected. So the removal list stays at all sixteen no matter how many survive, and the published check reads "`N` of `N` in the matched arm, zero of the sixteen in the control."

7. **The "what would pure chance have given" figure is calculated for the arm we actually built.** Amendment 5 asks for the number of CA1 shapes a purely region-blind draw of sixteen would have picked up. If the arm is `N`, that calculation uses `N`. The figures already recorded in Amendment 5 stay as they are — they were measured on the frozen donor table before any host existed, and they were never predictions.

**What this retires, and what it deliberately does not.** Wherever Amendments 2, 3 and 5 say "sixteen" to mean **the current size of a Tier A arm, set, rota or draw**, that reading is retired from today and becomes `N`. That includes Amendment 3's later sentence calling P1 a "fixed sixteen," which the first version of this amendment missed even though point 5 already gave it the new `N` rule. Old audit numbers and descriptions of what an earlier design actually measured at sixteen stay true history; they do not become current arm-size requirements and are not recalculated here. Amendment 2 is additionally narrowed in the one respect named in point 4: which round each turn falls in, which it left random, is now fixed. As always, the original text is not edited — amendments are added and dated, never rewritten. Three "sixteens" are deliberately left alone because they mean something different and are still true: that the CA1 library holds sixteen shapes in total, that all sixteen are eligible before host checks, and the removal list in point 6.

One knock-on worth naming: Amendment 4's limitation about the donor lab points at "the sixteen templates named in Slot 13.9." Since Slot 13.9 is being narrowed below, that pointer follows it automatically. It is mentioned here because a change to a design property has to be checked against every sentence that describes that property, not just the ones listed at the top.

**Does this change what counts as success or failure?** No threshold, no interval, no materiality line, no round count, no injected-cell count and no compute budget moves. The failure line at fewer than ten survivors is the one already written, restated.

**What does change is one limitation, and it gets sharper.** Slot 13.9 already said a Tier A result is a statement about those sixteen CA1 shapes and not about region-matched shapes in general. From now on it is a statement about **the exact survivors, with the killed ones listed** so a reader can see which cells the result does not speak for. The original reasoning gets stronger rather than weaker: with fewer donors the pool is exhausted even more completely, so running more rounds still cannot buy variety. At ten survivors every round contains all ten donors and there is **no variation between rounds at all** — which is the bluntest possible version of the point that limitation exists to make.
