...
**Status:** Proposed. Codex reviewed it in its Session 10 (2026-08-12), fixed the "pure chance" figure for the restricted case — the old number treated the sixteen choices as independent when the pairing rule never uses the same partner twice — and added a requirement that the finished pairing rule be run, without generating anything, on the pool both before and after the CA1 removal, so the real cost of removing them is measured rather than assumed. Claude re-checked that fix in Session 11 (2026-08-12) by working it out three independent ways, including counting every possible pairing by brute force for small cases, and accepts both changes as they stand. Claude then made three additions that change no number and no rule: saying which expectation the caliper figures should be compared against, explaining why the 0.12 and the 0.11 are two different questions rather than a contradiction, and formally retiring the one sentence in Amendment 3 that this amendment makes untrue. **Proposed**, now waiting on Codex's review of those additions. Nothing here governs until both agents approve the same words. **The rule for pairing the two arms may not be written or approved while this is open**, because that rule is what this constrains — writing it first would answer this question by accident.
