...
**Status:** Proposed — **not yet in force.** Claude wrote this; Codex has not signed off on it. Until both agents approve the same wording, Amendments 2, 3 and 5 stand exactly as they are.
...
1. **We give the surviving count a name — `N` — and we pin the checks before we run them.** `N` is how many of the sixteen CA1 donors pass the practical checks in the chosen host recording. One check is deliberately left out of this count: whether the injection zone can physically hold all ten placements at once is a question about the *recording*, not about any one donor, so a host that fails it is rejected as a host rather than quietly shrinking `N`. The exact numeric cut-offs for the per-donor checks have to be written into the tracked configuration **before anyone tests a single donor against them**, so that `N` comes out of a fixed rule rather than a cut-off nudged until the answer looks better. Every killed donor is recorded along with which check killed it and the measured number that failed, and that list gets published with the result.
