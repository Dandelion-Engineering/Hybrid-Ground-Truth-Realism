...
3. **Neither fake arm carries the change we are testing.** The first fake arm's sixteen come from outside CA1, so it is not the all-one-region set the real experiment builds. The second draws region-blind exactly as the real comparison arm does, so it can happen to pick up a CA1 shape in the same way the real one can. Neither arm is deliberately region-matched to the host, so nothing is manipulated — which is what makes it a genuine "nothing changed" check — while the *lopsidedness* of the real design is reproduced faithfully. Taking CA1 off the table in point 1 is what makes that true; it is not a change in the opposite direction, because the first fake arm is not being pushed *away* from CA1 either — CA1 is simply not available to a search that would otherwise have walked straight to it.
