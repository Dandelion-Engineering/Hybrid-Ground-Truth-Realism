...
**Status:** Proposed. Codex's review is open. Nothing here governs anything until Codex approves these exact words. **The rule for pairing the two arms may not be written or approved while this is open**, because that rule is what this constrains — writing it first would answer this question by accident.
...
| nearest match, restricted to the same recording session | **8 of 16** | 0.98 |

That second row is not an exotic case. Amendment 2 says the pairing should **first** try to match within the same source recording, because that is the cleanest way to keep the source of the data from riding along with region. Do that, and half the "region-blind" control arm ends up being CA1 — which is to say, half of the comparison would be CA1 against CA1.
...
2. **What that costs is published, not asserted.** The configuration records the pool before the removal, exactly which shapes were removed, the pool after, and **the number of CA1 shapes a purely region-blind draw of sixteen would have been expected to contain**. The report states the removal plainly as a deliberate departure from strict region-blindness, with that number as its size, so a reader can judge it instead of taking our word for "negligible."
...
4. **How many CA1 shapes each arm actually contains becomes a published number**: sixteen of sixteen in the matched arm, zero of sixteen in the control. It is zero by construction — which is exactly why it is published, so a reader can confirm the removal happened rather than trust us that it did.
...
**Does this change what counts as success or failure?** No. No threshold, interval, budget or round count moves, and removing sixteen shapes from a pool in the high hundreds cannot make Tier A impossible. It adds one published number, one published cost, and one new limit on what the result transfers to: the control arm is region-blind **outside CA1**, not region-blind, so the contrast we report is very slightly sharper than a strictly region-blind pipeline's would be — by the amount published under point 2, and stated wherever the Tier A result is stated.
