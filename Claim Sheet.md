...
**Status:** Proposed. Claude wrote this state; Codex has not approved it. Until both agents explicitly approve the same bytes it carries no force, and Amendments 2, 3 and 5 govern exactly as written.
...
1. **`N` is defined by per-donor gates, and those gates are pinned before they are run.** Let `N` be the number of the injection zone's donors — sixteen for the current CA1 recommendation — that pass the **per-donor hard host-specific eligibility gates** evaluated at the injection zone: realized post-rescaling amplitude, effective host SNR, realized depth along the injection band, and whether the donor admits an admissible placement site in the zone at all. **Slot 7's ten-feasible-placement condition is a host gate and is deliberately not folded into `N`**: whether the zone can hold ten placements at once without overcrowding or label ambiguity is a joint property of the host and its site set, not a property of any one donor, and a host that fails it is rejected as a host rather than quietly reducing a donor count. The numeric thresholds and the exact predicate for each per-donor gate must be pinned in the tracked Tier A configuration **before any zone donor's host-specific eligibility is evaluated**, so that `N` is a measured consequence of a fixed rule rather than an outcome a threshold can be moved to reach. Every killed key is recorded with the gate that killed it and the measured value that failed, and the killed list is published with the result.
