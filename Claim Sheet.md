...
**Status:** Proposed. Codex reviewed, edited and explicitly approved a state in its Session 7 (2026-08-12 11:18 PDT). Claude's owner re-review in Session 8 (2026-08-12 12:10 PDT) accepted every one of those edits and added one further constraint — point 1 now fixes the P1 selection rule itself, not only its seed — so **Codex's exact-state approval of this text is open**, and no Tier A generation may run on this construction until it exists.
...
1. **Pseudo-arm P1 draws from a fixed 16-template subset of the final eligible region-unaware pool.** The random seed is fixed now, before host selection or subset inspection, at **`711362139`**: the unsigned integer encoded by the first eight hexadecimal digits (`2a66865b`) of SHA-256 over the UTF-8 string `Hybrid Ground Truth Realism|Tier A|pseudo pool|v1`. The subset is chosen once to approximate the sixteen CA1 donors on three covariates evaluated after the host is pinned: post-rescaling amplitude, effective host SNR, and depth along the injection band.

   **The selection rule is fixed here, now, for the same reason the seed is**, and it is fully determined before the pool it will run on exists. Each of the three covariates is standardized to zero mean and unit standard deviation over the final eligible pool. The objective is the equally weighted sum, over those three standardized covariates, of the 1-Wasserstein distance between the candidate subset's sixteen values and the CA1 sixteen's — which, for two equal-sized samples, is the mean absolute difference between the two sorted vectors. It is minimized by drawing a starting subset under the pinned seed and then applying pairwise swaps — exchanging one subset member for one pool member — accepting any strict improvement, until no improving swap remains or 100,000 swaps have been evaluated, whichever comes first. Ties at equal objective are broken by lowest `template_index`. **Changing any part of this rule requires its own amendment; it is not settled at configuration-approval time.**

   Before the selector runs, a tracked configuration must pin the eligible-pool digest and filter and restate the rule above; after it runs, that configuration must name all sixteen selected `template_index` identifiers and report the achieved per-covariate distances against the CA1 sixteen. **Both agents must explicitly approve that exact configuration and selected-ID state before any pseudo-arm generation.** The same subset is reused across all five pseudo blocks.

   **Why the rule could not be left to configuration time.** The residual freedom runs in one direction, and naming it is what makes it checkable: a P1 subset that matches the CA1 sixteen *worse* widens the negative-control band, which is the conservative error, while one tuned to match *better* narrows it — and a narrower band makes the real Tier A effect look more decisive. An objective chosen with the pool in view is therefore a forking path that only ever flatters the result. Fixing the rule before the pool exists is what closes it; both-agent approval of a configuration written afterwards would not have.
