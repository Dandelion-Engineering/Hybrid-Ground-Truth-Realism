...
**Status:** Proposed. Codex's exact-state review is open. This amendment carries no force and nothing in it governs until Codex explicitly approves these bytes. **Tier A's donor-matching rule may not be fixed or approved while it is open**, because the rule is what it constrains, and fixing the rule first would settle this question by omission.
...
**Why it changes the path.** The obvious objection to removing the zone from the control arm is fidelity: the anchor pipeline draws region-blind, so a control that excludes the host's region is more mismatched than the field's actual practice, and that would be an inverse manipulation of the kind this project has refused twice. **The objection does not survive the arithmetic.** The anchor pipeline does not covariate-match its templates to a region-matched set — it has no region-matched set to match to. Ours does, because a paired design is how this project buys precision from one desktop instead of from N (Slot 4). So the pull toward the zone is manufactured by *our* pairing, not inherited from the method under test. What a genuinely region-blind draw would have contained is computable: 16 draws with 16 zone members in 2,183 gives **0.12 expected zone donors**, and about one arm in nine holding a single one. Removing them costs that and no more. Leaving them in costs, on the measured pull, between three and eight of sixteen — a control arm that is partly the treatment, which is the same defect Amendment 3 was written to remove from the pseudo-arms and the replicate construction was rejected for before that.
...
4. **The realized zone-donor count in each arm becomes a reported manipulation-check quantity** (Slot 11.3): sixteen of sixteen in the matched arm, zero of sixteen in the control after point 1. It is zero by construction, which is the point — it is reported so a reader can verify the removal happened rather than trust that it did.
...
**Success, failure and non-transfer shapes.** Slot 11.3 gains the reported quantity in point 4. Slot 12's failure shapes are unchanged: removing sixteen templates from a pool in the high hundreds cannot make Tier A infeasible, and 12.3's donor-feasibility collapse is unaffected. No threshold, interval, budget, block count, or sorter-hour ceiling moves.

**New non-transfer shape — Slot 13.11.** *Tier A's control arm is region-blind outside the injection zone, not region-blind.* The comparison this project reports is between region-matched donors and donors drawn without conditioning on region **from a pool the injection zone has been removed from**. That is a slightly sharper contrast than a strictly region-blind draw would produce, by the expectation reported in point 2, and it is stated wherever the Tier A result is stated. A reader applying this result to a pipeline that draws strictly region-blind should read the reported expectation as the size of the difference between the two designs.
