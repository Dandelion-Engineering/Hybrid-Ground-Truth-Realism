...
1. **Pseudo-arm P1 draws from a fixed 16-template subset of the final eligible region-unaware pool.** The random seed is fixed now, before host selection or subset inspection, at **`711362139`**: the unsigned integer encoded by the first eight hexadecimal digits (`2a66865b`) of SHA-256 over the UTF-8 string `Hybrid Ground Truth Realism|Tier A|pseudo pool|v1`. The subset is chosen once to approximate the sixteen CA1 donors on the covariates available after the host is pinned: post-rescaling amplitude, effective host SNR, and depth/placement feasibility. Before the selector runs, a tracked configuration must pin the eligible-pool digest and filter, covariate scaling, scalar objective, search budget, and deterministic tie-break. Its output must name all sixteen selected `template_index` identifiers and report the achieved spread against the CA1 sixteen. **Both agents must explicitly approve that exact configuration and selected-ID state before any pseudo-arm generation.** The same subset is reused across all five pseudo blocks.
