...
1. **Pseudo-arm P1 draws from a fixed 16-template subset of the final eligible region-unaware pool, with the injection zone's own donor pool removed from the search space.** For the approved CA1 zone that removal is exactly the sixteen templates that constitute the real matched arm. It is required, not tidiness: the region-unaware arm draws *without conditioning on region* (Slot 5, Tier A), so the CA1 sixteen are themselves members of the final eligible pool, and the objective defined below is minimized at distance zero by those sixteen. A search run over a space containing them is a search pointed at the real matched arm, and every template it recovers puts the manipulation inside the control — the exact failure the replicate construction was rejected for. The removal touches nothing else in this rule: not the seed, the covariates, the objective, the search, the rota, or the budget. It is decided here, before the pool exists, for the same reason the rest of the rule is. The configuration reports both counts, the final eligible pool and the search space remaining after the removal. A template is identified by the pair **(`dataset`, `template_index`)**, not by `template_index` alone: the integer resets inside every dataset (the tracked Neuropixels 1.0 snapshot has 2,183 rows but only 187 distinct integer values), while the pair is unique for all 2,183. This is also the unambiguous identifier intended wherever Slot 7's earlier shorthand says templates are pinned by `template_index`; all real- and pseudo-arm configurations use the pair. The random seed is fixed now, before host selection or subset inspection, at **`711362139`**: the unsigned integer encoded by the first eight hexadecimal digits (`2a66865b`) of SHA-256 over the UTF-8 string `Hybrid Ground Truth Realism|Tier A|pseudo pool|v1`. The subset is chosen once to approximate the sixteen CA1 donors on three covariates evaluated after the host is pinned: post-rescaling amplitude, effective host SNR, and depth along the injection band.
   ...
   **The selection rule is fixed here, now, for the same reason the seed is**, and its choices are determined before the pool it will run on exists. For each covariate, compute the final eligible pool's mean and population standard deviation (`ddof=0`) in IEEE-754 float64 and use those same two values to standardize both the eligible-pool values and the CA1 sixteen. A non-finite value or a zero standard deviation fails the selector-configuration gate rather than triggering an improvised fallback. The objective is the equally weighted sum, over those three standardized covariates, of the 1-Wasserstein distance between the candidate subset's sixteen values and the CA1 sixteen's — which, for two equal-sized samples, is the mean absolute difference between the two sorted vectors.
   ...
   Initialization and search are deterministic and bounded. For every (`dataset`, `template_index`) pair **in the search space defined above** — the final eligible pool minus the injection zone's donor pool — hash the exact UTF-8 string `711362139\n<dataset>\n<template_index>` with SHA-256; the starting subset is the sixteen smallest unsigned big-endian digests, with the identifier pair breaking a digest tie. One search sweep evaluates every one-for-one swap between the current subset and its complement. If no swap strictly lowers the objective, stop. Otherwise take the swap producing the lowest objective; equal objectives are broken by the lexicographically lowest sorted sixteen-pair result. Repeat only while another **complete** sweep keeps the cumulative number of evaluated swaps at or below 100,000; a partial sweep is never used. The configuration reports whether the search stopped at a local optimum or at the evaluation cap. This is a fixed, reproducible bounded search — **not a claim that the global optimum was found**. Changing any part of this rule requires its own amendment; it is not settled at configuration-approval time.

   Before the selector runs, a tracked configuration must pin the eligible-pool digest and filter and restate the rule above; after it runs, that configuration must name all sixteen selected (`dataset`, `template_index`) pairs, report the achieved per-covariate distances against the CA1 sixteen, and record the stopping reason and evaluated-swap count. **Both agents must explicitly approve that exact configuration and selected-ID state before any pseudo-arm generation.** The same subset is reused across all five pseudo blocks.
...
2. **Pseudo-arm P2 draws from that same full final eligible region-unaware pool**, covariate-matched to P1 by exactly the procedure that matches the real region-unaware arm to the real matched arm. No second matching method is introduced, and the final eligible-pool count is reported rather than replaced by the provisional 1,149-template screen.
3. **Neither pseudo-arm carries the region manipulation.** P1's fixed sixteen come from outside the injection zone's donor pool and are therefore not region-homogeneous with the host's injection zone; P2 draws without conditioning on region exactly as the real region-unaware arm does, and may therefore contain injection-zone templates by chance in precisely the way the real arm can. Neither half is region-matched to the injection zone, so nothing is manipulated and the arms remain "under the same nominal condition" in the sense Slot 5 requires. **Point 1's removal is a precondition of that sentence being true rather than a second manipulation running the other way:** it withholds the manipulation from P1, and it is not an inverse manipulation, because P1 is not selected to be *distant* from the injection zone — the removed region is simply unavailable to a search that would otherwise be drawn to it. What differs between P1 and P2 is pool size and donor reuse — the structural asymmetry of the real contrast — and nothing else.
