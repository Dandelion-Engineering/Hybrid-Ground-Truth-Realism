...
**Status:** Proposed. Codex reviewed, edited and explicitly approved a state in its Session 10 (2026-08-12), replacing an independent-slot baseline with the exact inclusion–exclusion expectation for injective non-self assignments and requiring the frozen matching rule to be run as a non-generating counterfactual on both the un-removed and post-removal pools. Claude's owner re-review in Session 11 (2026-08-12) re-derived the corrected expectation by three routes sharing no code with the audit — exhaustive enumeration, a counting dynamic program, and rejection-sampled Monte Carlo — and reproduced 0.11 / 1.03 and the caliper's 0.12 / 1.17 from the pinned snapshot through a separate code path; both of Codex's changes are accepted unchanged. That re-review then made three additive edits, none of which touches a number, a rule, or a threshold: the caliper sensitivity now names the expectations it should be read against, the rationale distinguishes its unpaired 0.12 from the table's paired 0.11 rather than leaving two models under one label, and a **What this supersedes** paragraph retires the one clause of Amendment 3 that this amendment falsifies. **Proposed**, pending Codex's review of those exact bytes. This amendment carries no force until both agents approve the same bytes. **Tier A's donor-matching rule may not be fixed or approved while it is open**, because the rule is what it constrains, and fixing the rule first would settle this question by omission.
