...
| **Baselines** | The anchor pipeline's exact default configuration as the control arm; and — the strongest non-signal control — a **seed-replicate null band**, the same nominal condition re-run under different random draws, which sets the floor any realism effect must clear. |
...
> **Holding the host recording, the injected unit identities, their amplitudes, their probe placements, their total spike counts, and the random seeds fixed, adding a named realism property to the standard SpikeInterface hybrid generator changes measured spike-sorting accuracy by a bounded amount and changes the between-sorter comparison by a bounded amount — and those bounds are either inside or outside the range that would change which sorter a reader would choose.**

The claim is transferable because the manipulation is defined against the pipeline the field actually uses, at a pinned commit, with everything except the manipulated property held fixed by construction. Anyone running the standard pipeline inherits the answer directly.
...
The constraints above rule out matching the anchor benchmark's scale — its authors estimated 870 and 846 single-workstation hours for their two sorter comparisons. This project buys its precision from **pairing** instead of from N: the same host recording, the same injected unit identities, the same placements, the same amplitude and SNR targets, the same total spike counts, and the same random seeds across arms, with exactly one property changed. A paired design on one desktop can resolve a difference that an unpaired design would need many machines to see.

### The control arm

The control is **the anchor pipeline's own default configuration**, reproduced exactly: templates drawn without regard to the host's brain region, Poisson spike times, a mean firing rate in the anchor's range, ten injected units per recording, the implemented refractory period, amplitudes rescaled into the anchor's 50–200 µV range, and motion applied by the same mechanism in every arm. Reproducing the default exactly is what makes the result inherited by anyone using the pipeline. Refractoriness is *already implemented upstream* and is therefore not one of our axes — it is part of the control.
...
- **Tier A — region-matched templates.** Donor templates are selected to match the host recording's brain area (matched arm) versus a maximally distant area (mismatched arm), with amplitude, SNR, probe geometry, and placement balanced across arms by caliper matching. Configuration only.
- **Tier B — local population-rate coupling.** Injected spike trains are generated as an inhomogeneous Poisson process driven by a population firing rate estimated from nearby host units, holding each unit's mean rate and refractory behaviour fixed. This is the anchor authors' own proposed remedy, and it has prior art: the Kilosort4 hybrid benchmark modulates exponential inter-spike intervals by local population rate in 100 ms bins.
- **Tier C — bursting with spike-history-dependent waveform attenuation.** Injected trains carry burst structure at biologically bounded parameters (inter-spike intervals down to ~6 ms within bursts, burst probability suppressed following recent firing), and the injected waveform amplitude is attenuated as a function of recent inter-spike interval. Both halves are required: timing-only bursts without the waveform coupling would be an incomplete manipulation, because the sorter-relevant mechanism is that a real neuron's waveform depends on its own firing history and a fixed average template's does not.
...
Three comparison points, and the third is the one that does the real work:
...
3. **The seed-replicate null band — the strongest non-signal control.** The same nominal condition is re-run under different random draws: different template subsets from within the same selection pool, different spike-time seeds, different placement seeds. This produces the distribution of accuracy differences and interactions attributable to *selection and seed noise alone, with no realism manipulation applied at all.* Any claimed realism effect must be shown against that band. If swapping which templates were drawn inside the matched condition moves the interaction as much as switching from matched to mismatched does, then the realism effect is indistinguishable from selection noise and is reported as such. **This band is the single most important control in the design**, because it is the one that separates a real effect from the appearance of one, and it is the control a reader is most likely to ask for.
...
### Host selection — now downstream of donor availability

An audit of the live template metadata in Session 2 reversed the expected selection order. Applying a caliper of **amplitude 50–200 µV** (adopted from the anchor's own rescaling range) and **SNR 5–15** to the 2,183 Neuropixels 1.0 templates leaves 1,149 templates across 149 brain areas, of which **37 areas hold at least 10 in-caliper templates** — ten being the anchor's injected-unit count per recording. That looks comfortable. But the donor/host leakage rule requires excluding the host recording's own source dataset from the donor pool, and after dropping each area's single largest contributing dataset, **only 7 areas retain at least 10 templates: CP (42), PIR (19), SUB (18), VISa5 (17), AId5 (14), MRN (14), ENTl5 (10).** Thirteen of the 37 collapse to zero, because one dataset supplied all of their templates.

So host selection is constrained by donor availability rather than the other way round, and the constraint is a conditional:

- if the chosen host is **not** one of the library's 37 IBL source datasets, all 37 areas are live;
- if it **is**, the region axis is restricted to the 7-area shortlist.

Both numbers move with the caliper, which is a declared parameter and not a fact; the audit is re-runnable at other calipers via `Reproducibility Packet/scripts/audit_template_library.py`.

**Host recordings are additionally screened** for: quantified drift below a pre-declared threshold (or, failing that, identical measured motion applied across every arm with a drift-following manipulation check), documented channel geometry, documented noise level, and adequate duration. Selected hosts are pinned by DANDI asset identifier. Selected templates are pinned by `template_index` against a metadata snapshot pinned by SHA-256 — the table is hosted mutably, and a result built on "latest" is not reproducible however good the analysis code is.

**Covariate balance across arms is a gate, not a hope.** Matched and mismatched arms are balanced on amplitude, SNR, probe geometry, depth/placement feasibility, **and the number of contributing source datasets**. That last one is not optional: excluding the host's own dataset bounds leakage but does not prevent provenance from riding along with region, and an arm drawn from five source datasets compared against an arm drawn from one is partly a provenance comparison wearing a region label.

### Injected-unit budget

Ten injected units per recording instance, matching the anchor. **Statistical power comes from paired arms, seed replicates, and multiple host recordings — never from raising the injected-unit count**, because more simultaneous injections change the collision and density statistics of the recording itself, which is the thing being held fixed.
...
| Tier | Passes when… |
|---|---|
| **A — region** | pre-declared multichannel waveform features separate matched from mismatched donor sets, while amplitude, SNR, probe geometry, placement, and source-dataset count remain balanced. Features chosen so as not to be simply the sorters' own decision variables. |
| **B — population coupling** | the injected units' rate trajectory tracks the locally estimated host population rate to a pre-declared similarity criterion, while mean firing rate and refractory-violation rate are unchanged from control. |
| **C — burst / history** | the short-ISI distribution and the history-dependent amplitude attenuation both fall inside pre-declared biological bounds, while the long-timescale mean rate is unchanged from control. |
...
For injected unit *u*, host *r*, sorter *s*, and realism condition *c*, with accuracy *A*:

- the **paired realism effect** for a sorter is `Δ_s = A(u,r,s,realistic) − A(u,r,s,control)`;
- the **primary comparative estimand** is the paired **difference in differences**, `I(s1,s2) = Δ_s1 − Δ_s2`.
...
Uncertainty comes from a **paired bootstrap over injected units, clustered by recording instance**, reported as 95% intervals on the raw quantities. **Decisions are pre-declared in raw paired accuracy and rank units with their uncertainty; standardized effect sizes are reported secondarily and never as thresholds.**
...
**Panel 1 — "Did the knob actually turn?"** For one injected unit, shown twice — once as the control arm generated it and once as the realistic arm did — three things side by side: a raw voltage snippet around several spikes, the inter-spike-interval histogram, and spike amplitude plotted against the preceding interval. The director does not need to know what a template is to see that the left column's amplitude is flat against interval and the right column's is not, or that the left column's intervals are featureless and the right column's cluster at a few milliseconds. **If the two columns look the same, the manipulation did nothing and nothing downstream matters** — which is exactly the stop-or-go gate, rendered so a non-specialist can apply it.

**Panel 2 — "Did it change the answer?"** The same units' accuracy scores under each sorter, control versus realistic, with the two sorters' paired gap drawn in both conditions and **the seed-replicate null band shaded behind them**. The question the director answers by looking: does the change between conditions stick out of the grey band, or sit inside it? An effect inside the band is noise. An effect outside it is real.

**What the director does.** Run one command, look at two panels, read one printed sentence of the form *"Under [tier], the gap between [sorter A] and [sorter B] moved by X accuracy points (95% CI …), against a seed-noise band of ±Y. The ranking did / did not reverse; the separation was / was not lost."* Then, if he wants, change a flag and re-run it on a different tier.
...
There is no trained model here, so "capacity" is the scale of the experiment: segment length, number of hosts, number of seed replicates, and number of sorters. The ladder below governs it. **The smallest-sufficient rule applies to what the project finally ships and reports, not to the search that gets there** — a null from an undersized experiment is evidence about that experiment, not about whether the effect exists.
...
| **2 — Primary run** | ~10 min segment, 1 host, 10 units, ≥5 seed replicates per arm, 2 sorters | Produce the first bounded interaction estimate and the seed-replicate null band | Interval computed |
...
**Climb the ladder when** (a) there is partial signal worth resolving, **or** (b) there is no signal yet *and* the interval is wide enough to still admit a decision-relevant effect. Rung 4 exists specifically for case (b): a null with a wide interval is inconclusive, not negative, and the response is more precision rather than a conclusion.
...
**The pilot's own budget is pre-declared** so that a marginal result does not become a negotiation: a candidate sorter is admitted if it completes the 60 s segment within a wall-clock and peak-memory budget recorded before the pilot runs, measured against free RAM and VRAM taken *at that moment*. A sorter that does not fit is dropped, named in the report as dropped, and the reason recorded. **If no third sorter fits, the project runs two and the narrower panel becomes a stated limitation** — pre-committing to the smaller honest panel now is what keeps it from being rationalized later.
...
3. a **95% interval on the sorter × realism interaction** exists, computed by paired bootstrap over injected units clustered by recording instance;
4. that interval is **narrow enough to sit either wholly inside or wholly outside the decision-relevant band**, so the result actually decides something; and
5. the **seed-replicate null band** was estimated from the same design, so the effect is judged against selection-and-seed noise rather than against zero.
...
- **Comparative materiality — any one of three events, each requiring its 95% interval to exclude zero:**
  - **Reversal.** The sign of the paired sorter difference flips between control and realistic arms.
  - **Loss (or gain) of separation.** The paired sorter gap's interval excludes zero in one arm and includes it in the other. *This is a distinct event from reversal and is the likelier of the two* — a comparison going from "clear winner" to "indistinguishable" changes a reader's conclusion just as much as one that reverses.
  - **Magnitude.** The interaction's magnitude reaches **≥ 50% of the control-arm paired sorter gap** measured in this experiment. Rationale: a shift of half the gap is enough to move a comparison a reader would call decisive into one they would call marginal. The 50% figure is a declared judgement, and it is declared here rather than after seeing results.

**A clean negative satisfies this slot completely.** If the manipulation demonstrably turned, the interval is tight, and it sits inside the band's null region — no reversal, no loss of separation, magnitude well under half the sorter gap — then the project has closed a stated open question, validated the field's default proxy, and supplied independent evidence for a position a maintainer has asserted in print without it. **That is a success and is published on exactly the same terms as a positive result.**
...
3. **Donor feasibility collapses.** No candidate host recording admits both a matched and a balanced mismatched arm without confounding amplitude, SNR, geometry, or provenance. Tier A is dropped, the reason published, and the project proceeds on Tiers B and C.
...
5. **One host means no cross-region or cross-probe transfer.** A result from a single host recording, brain area, and probe type is a result about that configuration. Generalization requires Rung 4 of the ladder, and absent it the claim carries the boundary.
...
7. **Non-transfer to real ground truth is assumed throughout.** Nothing here substitutes for paired juxtacellular/extracellular recordings, which answer a different and harder question. A finding that a hybrid benchmark is or is not realism-sensitive says nothing directly about how either compares to real ground truth.
...
- the **seed-replicate null band**, plotted alongside every effect;
...
**The minimum scientific content required to conclude:** Tier A and Tier B each carried to either a bounded interaction estimate or a documented Slot 12 failure, with manipulation checks reported either way. **Tier C is required if its generator passes its manipulation check, and is reported as an attempted-and-bounded axis if it does not.** The project may not conclude on Tier A alone (Slot 13.2).
